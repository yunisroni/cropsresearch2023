...
```
```{r}
ts_deli1 <- ts(Var2_deli, start = c(2015,5), frequency = 1)
head(ts_deli1)
```
...
```{r}
# VAR Model
Model_var_deli <- VAR(ts_deli1, p = lag1_deli$selection[1], type = "both") # jumlah order dari model VAR
summary(Model_var_deli)
```
...
```{r}
# Melakukan uji Granger Causality
granger_padi_deli1 <- causality(var_padi_deliserdang, cause = "Luas_Panen")
granger_padi_deli1
```
```{r}
# Hitung Pearson correlation coefficient
...
# Nilai correlation coefficient
cat("Koefisien Korelasi Pearson Luas Panen dan Produksi:", pearson_curahhujan_deli)
```
```{r}
# Hitung Pearson correlation coefficient
...
# Nilai correlation coefficient
cat("Koefisien Korelasi Pearson Luas Panen dan Produksi:", pearson_harihujan_deli)
```
