...
```{r}
# Merubah nama variabel
names(Hujan_kab)[names(Hujan_kab) == "Kabupaten/ Kota"] <- "kabupaten_kota"
names(Hujan_kab)[names(Hujan_kab) == "Curah Hujan (mm)"] <- "Curah_hujan"
names(Hujan_kab)[names(Hujan_kab) == "Hari Hujan (hari)"] <- "Hari_hujan"
tail(Hujan_kab)
```
### Merubah type data
```{r}
# Merubah type variabel Tahun menjadi Date
Hujan_kab$Tahun <- as.Date(Hujan_kab$Tahun)
glimpse(Hujan_kab)
```
...
```{r}
# Menghapus data NA's
Hujan_kab_clean <- na.omit(Hujan_kab)
summary(Hujan_kab_clean)
```

```{r}
colSums(is.na(Hujan_kab_clean))
```
```{r}
#Filter data Hujan hanya Deli serdang saja
Hujan_Deli <- Hujan_kab_clean %>%
  filter(kabupaten_kota == "Deli Serdang")
Hujan_Deli
```
...
```{r}
# Filter data for kabupaten hanya deli serdang saja
