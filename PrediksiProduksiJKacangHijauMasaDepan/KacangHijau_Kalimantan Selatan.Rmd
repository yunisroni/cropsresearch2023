---
title: "Prediksi Variabel Yang Mempengaruhi Produksi Kacang Hijau Masa Depan - Kalimantan
  Selatan"
...
## a.Import Data
```{r}
pangan <- read_excel("Dataset Kacang Hijau Indonesia/pangan_Indonesia.xlsx")
```
...
```
Jumlah data setelah dihapus data NA' menjadi 4,642 baris observasi dengan
6 variabel.
...
## c. Jumlah Produksi Kacang Hijau Per Provinsi
```{r}
...
## d. Tren Produksi Kacang Hijau Per Provinsi
```{r}
...
## e. Perbandingan Produksi Kacang Hijau Per Provinsi
```{r}
...
```
Perbandingan produksi kacang hiaju dalam kurun waktu 1993 - 2015 adalah dari provinsi Jawa Tengah yang terbanyak dengan Jumlah Produksi kacang hiaju sebesar 1549775 Ton.

## f. Perbandingan Produktivitas kacang hiaju Per Provinsi
```{r}
# Turn off dplyr summarise() warning
options(dplyr.summarise.inform = FALSE)
...
```
Berdasarkan grafik diatas bisa lihat bahwa ada korelasi antara luas panen dan produksi. Hubungan korelasi adalah positif dengan nilai korelasi sebesar 0,96; sehingga bisa disimpulkan bahwa jika semakin banyak luas panen maka akan meningkat jumlah produksi jagung.
```{r}
# Visualisasi korelasi produktifitas dan produksi Kacang Hijau
plot_kacanghijau_6 <- ggplot(KacangHijau, aes(x = Produktivitas, y = Produksi, color = Provinsi)) +
  geom_point() +
...
```{r}
# Hitung Pearson correlation coefficient
correlation_kacanghijau2 <- cor(KacangHijau$Produktivitas, KacangHijau$Produksi)

# Nilai correlation coefficient
cat("Koefisien Korelasi Pearson Produktivitas dan Produksi:", correlation_kacanghijau)
```
Berdasarkan grafik diatas bisa lihat bahwa ada korelasi antara produktivitas dan produksi. Hubungan korelasi adalah positif dengan nilai korelasi sebesar 0.96; Nilai korelasi sebesar 0.96 menunjukkan hubungan positif yang relatif kuat. Ini berarti bahwa meskipun ada hubungan antara produktivitas dan produksi, hubungannya sangat kuat.

## h. Filter data Kacang Hijau
```{r}
KacangHijauKalimantanSelatan <- filter(pangan_clean, Jenis_Komoditi == "Kacang Hijau", Provinsi == "KALIMANTAN SELATAN")
KacangHijauKalimantanSelatan
...
train_data_index_lp <- createDataPartition(KacangHijauKalimantanSelatan$Luas_Panen, p=0.7, list = FALSE)
train_data_lp <- KacangHijauKalimantanSelatan[train_data_index_lp,]
train_data_lp
test_data_lp <- KacangHijauKalimantanSelatan[-train_data_index_lp,]
test_data_lp
```
...
future_prophet_ts_lp <- make_future_dataframe(prophet_ts_lp, periods = 365*13)
...
```{r}
# membagi data berdasarkan variabel Produksi
train_data_index_pr <- createDataPartition(KacangHijauKalimantanSelatan$Produktivitas, p=0.7, list = FALSE)
train_data_pr <- KacangHijauKalimantanSelatan[train_data_index_pr,]
train_data_pr
test_data_pr <- KacangHijauKalimantanSelatan[-train_data_index_pr,]
test_data_pr
```
...
```
Mengambil data produktivitas dari hasil prediksi 2016-2025
```{r}
# Filter data untuk tahun 2021-2024
forecast_subset_pr <- forecast_ts_pr[forecast_ts_pr$ds >= as.Date("2016-01-01") & forecast_ts_pr$ds <= as.Date("2025-12-31"), ]
