---
title: "Prediksi Variabel Yang Mempengaruhi Produksi Kacang Hijau Masa Depan - Kep
  Riau"
...
```{r}
KacangHijauKepRiau <- filter(pangan_clean, Jenis_Komoditi == "Kacang Hijau", Provinsi == "KEP. RIAU")
KacangHijauKepRiau
...
train_data_index_lp <- createDataPartition(KacangHijauKepRiau$Luas_Panen, p=0.6, list = FALSE)
train_data_lp <- KacangHijauKepRiau[train_data_index_lp,]
train_data_lp
test_data_lp <- KacangHijauKepRiau[-train_data_index_lp,]
test_data_lp
```
...
future_prophet_ts_lp <- make_future_dataframe(prophet_ts_lp, periods = 365*19)
...
```{r}
# membagi data berdasarkan variabel Produksi
train_data_index_pr <- createDataPartition(KacangHijauKepRiau$Produktivitas, p=0.6, list = FALSE)
train_data_pr <- KacangHijauKepRiau[train_data_index_pr,]
train_data_pr
test_data_pr <- KacangHijauKepRiau[-train_data_index_pr,]
test_data_pr
```
...
```{r}
# Melakukan Prediksi untuk produksi 10 tahun ke depan
future_prophet_ts_pr <- make_future_dataframe(prophet_ts_pr, periods = 365*20)
