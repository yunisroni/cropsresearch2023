...
data: residuals_var W = 0.3585, p-value \< 2.2e-16 Hasil dari uji Shapiro-Wilk menunjukkan nilai p-value yang rendah,yang menunjukkan bahwa data residual tidak normal. Hal ini dapat menyebabkan kesalahan pada proses pemodelan, karena model GARCH mengasumsikan bahwa data residual normal.
...
```{r}
# Menyimpan hasil prediksi produktivitas
...
```{r}
# Menyimpan hasil prediksi avg rainfal
...
```{r}
# Menyimpan hasil prediksi produksi
prediksi_garch_produksi_j <- ugarchforecast(garch_produksi_jagung, n.ahead = 10)
prediksi_garch_produksi_j
```

```{r}
# Mengambil nilai prediksi GARCH Produksi, produktivitas dan rainfall dan membuat menjadi data frame
prediksi_garch_jagung <- data.frame(Produktivitas = as.numeric(prediksi_garch_ptj@forecast$seriesFor[, 1]),
                                    Avg_Rainfall = as.numeric(prediksi_garch_avg@forecast$seriesFor[, 1]),
                                    Produksi = as.numeric(prediksi_garch_produksi_j@forecast$seriesFor[, 1]))
...
cat("MAPE Produksi jagung:", mape_garch_j, "%\n")
cat("RMSE Produksi jagung:", rmse_garch_j, "\n")
cat("MAE Produksi jagung:", mae_garch_j, "\n")
cat("MSE Produksi jagung:", mse_garch_j, "\n")
```
...
cat("MAPE Produktivitas jagung:", mape_garch_ptj, "%\n")
cat("RMSE Produktivitas jagung:", rmse_garch_ptj, "\n")
cat("MAE Produktivitas jagung:", mae_garch_ptj, "\n")
cat("MSE Produktivitas jagung:", mse_garch_ptj, "\n")
```
MAPE Produktivitas jagung: 100 %
RMSE Produktivitas jagung: 53.53347 
MAE Produktivitas jagung: 51.566 
MSE Produktivitas jagung: 2865.832 
...
Multiple R-Squared: 0.6857, F-statistic: 32.99 p-value: \< 2.2e-16
...
Multiple R-Squared: 0.1746,F-statistic: 3.198 , p-value: 5.224e-05

Estimation results for equation Produkivitas: 
Produktivitas.l1 5.199e-01 6.532e-02 7.960 6.66e-14 
Multiple R-Squared: 0.3855, F-statistic: 9.487, p-value: \< 2.2e-16
...
prediksi_garch_kedelai <- data.frame(Produktivitas = as.numeric(Prediksi_garch_pt_kedelai@forecast$seriesFor[, 1]),
                                    avg_rainfall = as.numeric(prediksi_garch_avgkk@forecast$seriesFor[, 1]),
                                    Produksi = as.numeric(prediksi_garch_produksi_kk@forecast$seriesFor[, 1]))
...
# Menampilkan plot
print(plot_predik_produksi_kedelai)
```
...
plot_predik_avgkk<- ggplot(prediksi_garch_kedelai
  ...
  geom_line() +
  geom_point() +
  labs(title = "Prediksi avg rainfall kedelai Sumatera (2023-2033)",
       x = "Tahun",
       y = "Produksi kedelai",
       color = "Metode Prediksi") +
  scale_y_continuous(labels = scales::comma) +
...
cat("MAPE Produksi Kedelai:", mape_garch_kk, "%\n")
cat("RMSE Produksi Kedelai:", rmse_garch_kk, "\n")
cat("MAE Produksi Kedelai:", mae_garch_kk, "\n")
cat("MSE Produksi Kedelai:", mse_garch_kk, "\n")
```
...
```{r}
prediksi_ptkk <- as.numeric(prediksi_garch_kedelai$Produktivitas)
```

```{r}
# Menghitung nilai RMSE, MAPE, MAE, dan MSE
rmse_garch_ptkk <- sqrt(mean((data_aktual_ptkk - prediksi_ptkk)^2))
mape_garch_ptkk <- mean(abs((data_aktual_ptkk - prediksi_ptkk) / data_aktual_ptkk) * 100)
mae_garch_ptkk <- mean(abs(data_aktual_ptkk - prediksi_ptkk))
mse_garch_ptkk <- mean((data_aktual_ptkk - prediksi_ptkk)^2)

cat("MAPE Produktivitas Kedelai:", mape_garch_ptkk, "%\n")
cat("RMSE roduktivitas Kedelai:", rmse_garch_ptkk, "\n")
cat("MAE roduktivitas Kedelai:", mae_garch_ptkk, "\n")
cat("MSE roduktivitas Kedelai:", mse_garch_ptkk, "\n")
```
...
1.  Dalam model VAR, bahwa variabel avg_rainfall berdampak pada produktivitas kacang karena nilai P-Value dari 2 lag yaitu Lag4 dan lag5 lebih rendah dari 0,05. dan avg_rainfall  berdampak pada produksi di lag 4 karena nilai P-Value lebih besar dari 0,05
...
```{r}
# Mengambil nilai prediksi GARCH dan membuat menjadi data frame
prediksi_garch_kacanghijau <- data.frame(Produktivitas = as.numeric(Prediksi_pt_kh@forecast$seriesFor[, 1]),
                                    avg_rainfall = as.numeric(prediksi_avg_kh@forecast$seriesFor[, 1]),
                                    Produksi = as.numeric(prediksi_produksi_kh@forecast$seriesFor[, 1]))
...
# Menampilkan plot
print(plot_predik_produksi_kacanghijau)
```
...
plot_predik_avgkk<- ggplot(prediksi_garch_kacanghijau
  ...
  geom_line() +
  geom_point() +
  labs(title = "Prediksi rainfall kacanghijau Sumatera (2023-2033)",
       x = "Tahun",
       y = "Produksi kacanghijau",
       color = "Metode Prediksi") +
  scale_y_continuous(labels = scales::comma) +
...
```{r}
# Trim data data aktual agar dapat diukur perfomanya
data_aktual_kacanghijau <- head(data_aktual_kh, length(prediksi_produksi_kh))
data_aktual_kacanghijau
```

```{r}
# Menghitung nilai RMSE, MAPE, MAE, dan MSE
rmse_garch_kh <- sqrt(mean((data_aktual_kacanghijau - prediksi_produksi_kh)^2))
mape_garch_kh <- mean(abs((data_aktual_kacanghijau - prediksi_produksi_kh) / data_aktual_kacanghijau) * 100)
mae_garch_kh <- mean(abs(data_aktual_kacanghijau - prediksi_produksi_kh))
mse_garch_kh <- mean((data_aktual_kacanghijau - prediksi_produksi_kh)^2)
cat("MAPE Produksi kacang hijau :", mape_garch_kh, "%\n")
cat("RMSE Produksi kacang hijau :", rmse_garch_kh, "\n")
cat("MAE Produksi kacang hijau :", mae_garch_kh, "\n")
cat("MSE Produksi kacang hijau :", mse_garch_kh, "\n")
```
MAPE Produksi kacang hijau : 98.93298 %
RMSE Produksi kacang hijau : 886.0209 
MAE Produksi kacang hijau : 760.5371 
MSE Produksi kacang hijau : 785033.1

```{r}
# Mengambil data produktivitas kacanghijau selama 10 tahun terakhir
data_aktual_ptkh <- Sumatera_kacang_hijau[format(Sumatera_kacang_hijau$Tahun, "%Y") >= "2013", "Produktivitas"]
data_aktual_ptkh
```
...
```{r}
# Trim data data aktual agar dapat diukur perfomanya
data_aktual_ptkh <- head(data_aktual_ptkh, length(prediksi_produksi_ptkh))
data_aktual_ptkh
```
...
cat("MAPE Produktivitas kacang hijau:", mape_garch_ptkh, "%\n")
cat("RMSE Produktivitas kacang hijau :", rmse_garch_ptkh, "\n")
cat("MAE Produktivitas kacang hijau:", mae_garch_ptkh, "\n")
cat("MSE Produktivitas kacang hijau:", mse_garch_ptkh, "\n")
```

```{r}
# Mengambil data avg rainfall kacanghijau selama 10 tahun terakhir
data_aktual_rainfal_kh <- Sumatera_kacang_hijau[format(Sumatera_kacang_hijau$Tahun, "%Y") >= "2013", "avg_rainfall"]
data_aktual_rainfal_kh
```

```{r}
data_aktual_rainfal_kh<- as.numeric(data_aktual_rainfal_kh$avg_rainfall)
```

```{r}
prediksi_rainfal_kh <- as.numeric(prediksi_garch_kacanghijau$avg_rainfall)
prediksi_rainfal_kh
```
```{r}
# Trim data data aktual agar dapat diukur perfomanya
data_aktual_rainfal_kh <- head(data_aktual_rainfal_kh, length(prediksi_rainfal_kh))
data_aktual_rainfal_kh
```

```{r}
# Menghitung nilai RMSE, MAPE, MAE, dan MSE
rmse_rainfall_kh <- sqrt(mean((data_aktual_rainfal_kh - prediksi_rainfal_kh)^2))
mape_rainfall_kh <- mean(abs((data_aktual_rainfal_kh - prediksi_rainfal_kh) / data_aktual_rainfal_kh) * 100)
mae_rainfall_kh <- mean(abs(data_aktual_rainfal_kh - prediksi_rainfal_kh))
mse_rainfall_kh <- mean((data_aktual_rainfal_kh - prediksi_rainfal_kh)^2)

cat("MAPE :", mape_rainfall_kh, "%\n")
...
```{r}
# Transformasi residual menggunakan transformasi logaritma
residu_var_KT$Produksi_log <- log(abs(residu_var_KT$Produksi) + 1)
```
...
```{r}
# Mengambil nilai prediksi GARCH dan membuat menjadi data frame
prediksi_garch_kacangtanah <- data.frame(Produktivitas = as.numeric(Prediksi_pt_kt@forecast$seriesFor[, 1]),
                                    avg_rainfall = as.numeric(prediksi_avg_kt@forecast$seriesFor[, 1]),
                                    Produksi = as.numeric(prediksi_produksi_kt@forecast$seriesFor[, 1]))
...
plot_predik_produksi <- ggplot(prediksi_garch_kacangtanah
  ...
  geom_line() +
  geom_point() +
  labs(title = "Prediksi Rainfall kacang tanah Sumatera (2023-2033)",
       x = "Tahun",
...
cat("MAPE Produksi kacang tanah:", mape_garch_kt, "%\n")
cat("RMSE Produksi kacang tanah:", rmse_garch_kt, "\n")
cat("MAE Produksi kacang tanah:", mae_garch_kt, "\n")
cat("MSE Produksi kacang tanah:", mse_garch_kt, "\n")
```
...
cat("MAPE Produktivitas kacang tanah:", mape_garch_ptkt, "%\n")
cat("RMSE Produktivitas kacang tanah:", rmse_garch_ptkt, "\n")
cat("MAE Produktivitas kacang tanah:", mae_garch_ptkt, "\n")
cat("MSE Produktivitas kacang tanah:", mse_garch_ptkt, "\n")
```
MAPE Produktivitas kacang tanah: 100 %
RMSE Produktivitas kacang tanah: 12.86906 
MAE Produktivitas kacang tanah: 12.6667 
MSE Produktivitas kacang tanah: 165.6128 
...
cat("MAPE rainfall kacang tanah:", mape_garch_rainfall, "%\n")
cat("RMSE rainfall kacang tanah:", rmse_garch_rainfall, "\n")
cat("MAE rainfall kacang tanah:", mae_garch_rainfall, "\n")
cat("MSE rainfall kacang tanah:", mse_garch_rainfall, "\n")
```
MAPE rainfall kacang tanah: 100 %
RMSE rainfall kacang tanah: 7.835492 
MAE rainfall kacang tanah: 7.745103 
MSE rainfall kacang tanah: 61.39494 
...
Terdapat avg_Rainfall terhadap produksi dan produktivitas, Berdasarkan hasil analisis dengan model VAR, analisis kausalitas, dan regresi linear ini, kita dapat menyimpulkan bahwa:
...
```{r}
# Mengambil nilai prediksi GARCH dan membuat menjadi data frame
prediksi_garch_ubijalar <- data.frame(Produktivitas = as.numeric(prediksi_pt_uj@forecast$seriesFor[, 1]),
                                    Avg_Rainfall = as.numeric(prediksi_avg_uj@forecast$seriesFor[, 1]),
                                    Produksi = as.numeric(prediksi_produksi_uj@forecast$seriesFor[, 1]))
...
```{r}
# Visualisasi tren produksi kedelai
plot_predik_rainfall_uj <- ggplot(prediksi_garch_ubijalar
                             , aes(x = Tahun, y = Avg_Rainfall, color = "Prediksi")) +
  geom_line() +
  geom_point() +
  labs(title = "Prediksi Rainfall ubijalar Sumatera (2023-2033)",
       x = "Tahun",
       y = "Produksi ubijalar",
       color = "Metode Prediksi") +
  scale_y_continuous(labels = scales::comma) +
...
cat("MAPE Produksi ubi jalar:", mape_garch_uj, "%\n")
cat("RMSE Produksi ubi jalar:", rmse_garch_uj, "\n")
cat("MAE Produksi ubi jalar:", mae_garch_uj, "\n")
cat("MSE Produksi ubi jalar:", mse_garch_uj, "\n")
```

```{r}
# Mengambil data produktivitas ubijalar selama 10 tahun terakhir
data_aktual_ptuj <- Sumatera_ubi_jalar[format(Sumatera_ubi_jalar$Tahun, "%Y") >= "2013", "Produktivitas"]
data_aktual_ptuj
```

```{r}
data_aktual_ptuj<- as.numeric(data_aktual_ptuj$Produktivitas)
```

```{r}
prediksi_produksi_ptuj <- as.numeric(prediksi_garch_ubijalar$Produktivitas)
```

```{r}
# Menghitung nilai RMSE, MAPE, MAE, dan MSE
rmse_garch_ptuj <- sqrt(mean((data_aktual_ptuj - prediksi_produksi_ptuj)^2))
mape_garch_ptuj <- mean(abs((data_aktual_ptuj - prediksi_produksi_ptuj) / data_aktual_ptuj) * 100)
mae_garch_ptuj <- mean(abs(data_aktual_ptuj - prediksi_produksi_ptuj))
mse_garch_ptuj <- mean((data_aktual_ptuj - prediksi_produksi_ptuj)^2)

cat("MAPE Produktivitas ubi jalar:", mape_garch_ptuj, "%\n")
cat("RMSE Produktivitas ubi jalar:", rmse_garch_ptuj, "\n")
cat("MAE Produktivitas ubi jalar:", mae_garch_ptuj, "\n")
cat("MSE Produktivitas ubi jalar:", mse_garch_ptuj, "\n")
```
...
```{r}
data_aktual_rainfal_uj<- as.numeric(data_aktual_rainfal$avg_rainfall)
data_aktual_rainfal_uj
```

```{r}
prediksi_rainfal_uj <- as.numeric(prediksi_garch_ubijalar$Avg_Rainfall)
prediksi_rainfal_uj
```

```{r}
# Trim data data aktual agar dapat diukur perfomanya
data_aktual_rainfal_uj <- head(data_aktual_rainfal_uj, length(prediksi_rainfal_uj))
data_aktual_rainfal_uj
```

```{r}
# Menghitung nilai RMSE, MAPE, MAE, dan MSE
rmse_garch_rainfall <- sqrt(mean((data_aktual_rainfal_uj - prediksi_rainfal_uj)^2))
mape_garch_rainfall <- mean(abs((data_aktual_rainfal_uj - prediksi_rainfal_uj) / data_aktual_rainfal_uj) * 100)
mae_garch_rainfall <- mean(abs(data_aktual_rainfal_uj - prediksi_rainfal_uj))
mse_garch_rainfall <- mean((data_aktual_rainfal_uj - prediksi_rainfal_uj)^2)

cat("MAPE Rainfall ubi jalar:", mape_garch_rainfall, "%\n")
cat("RMSE Rainfall ubi jalar:", rmse_garch_rainfall, "\n")
cat("MAE Rainfall ubi jalar:", mae_garch_rainfall, "\n")
cat("MSE Rainfall ubi jalar:", mse_garch_rainfall, "\n")
```
...
```{r}
# Mengambil nilai prediksi GARCH dan membuat menjadi data frame
prediksi_garch_ubikayu <- data.frame(Produktivitas = as.numeric(prediksi_pt_uk@forecast$seriesFor[, 1]),
                                    Avg_Rainfall = as.numeric(prediksi_avg_uk@forecast$seriesFor[, 1]),
                                    Produksi = as.numeric(prediksi_produksi_uk@forecast$seriesFor[, 1]))
...
```{r}
# Visualisasi tren produksi kedelai
plot_predik_rainfall_uk <- ggplot(prediksi_garch_ubikayu
                             , aes(x = Tahun, y = Avg_Rainfall, color = "Prediksi")) +
  geom_line() +
  geom_point() +
  labs(title = "Prediksi Rainfall ubikayu Sumatera (2023-2033)",
       x = "Tahun",
       y = "Produksi ubikayu",
       color = "Metode Prediksi") +
  scale_y_continuous(labels = scales::comma) +
...
data_aktual_uk <- Sumatera_ubi_kayu[format(Sumatera_ubi_kayu$Tahun, "%Y") >= "2013", "Produksi"]
data_aktual_uk
```
...
cat("MAPE Produksi Ubi Kayu:", mape_garch_uk, "%\n")
cat("RMSE Produksi Ubi kayu:", rmse_garch_uk, "\n")
cat("MAE Produksi Ubi kayu:", mae_garch_uk, "\n")
cat("MSE Produksi Ubi kayu:", mse_garch_uk, "\n")
```

```{r}
# Mengambil data produktivitas ubikayu selama 10 tahun terakhir
data_aktual_ptuk <- Sumatera_ubi_kayu[format(Sumatera_ubi_kayu$Tahun, "%Y") >= "2013", "Produktivitas"]
data_aktual_ptuk
```

```{r}
data_aktual_ptuk<- as.numeric(data_aktual_ptuk$Produktivitas)
```

```{r}
prediksi_produksi_ptuk <- as.numeric(prediksi_garch_ubikayu$Produktivitas)
```

```{r}
# Menghitung nilai RMSE, MAPE, MAE, dan MSE
rmse_garch_ptuk <- sqrt(mean((data_aktual_ptuk - prediksi_produksi_ptuk)^2))
mape_garch_ptuk <- mean(abs((data_aktual_ptuk - prediksi_produksi_ptuk) / data_aktual_ptuk) * 100)
mae_garch_ptuk <- mean(abs(data_aktual_ptuk - prediksi_produksi_ptuk))
mse_garch_ptuk <- mean((data_aktual_ptuk - prediksi_produksi_ptuk)^2)

cat("MAPE Produktivitas Ubi kayu:", mape_garch_ptuk, "%\n")
cat("RMSE Produktivitas Ubi kayu:", rmse_garch_ptuk, "\n")
cat("MAE Produktivitas Ubi kayu:", mae_garch_ptuk, "\n")
cat("MSE Produktivitas Ubi kayu:", mse_garch_ptuk, "\n")
```

```{r}
# Mengambil data avg rainfall ubikayu selama 10 tahun terakhir
data_aktual_rainfal <- Sumatera_ubi_kayu[format(Sumatera_ubi_kayu$Tahun, "%Y") >= "2013", "avg_rainfall"]
data_aktual_rainfal
```

```{r}
data_aktual_rainfall_uk<- as.numeric(data_aktual_rainfal$avg_rainfall)
```

```{r}
prediksi_rainfal_uk <- as.numeric(prediksi_garch_ubikayu$Avg_Rainfall)
```

```{r}
# Menghitung nilai RMSE, MAPE, MAE, dan MSE
rmse_garch_rainfall_uk <- sqrt(mean((data_aktual_rainfall_uk - prediksi_rainfal_uk)^2))
mape_garch_rainfall_uk <- mean(abs((data_aktual_rainfall_uk - prediksi_rainfal_uk) / data_aktual_rainfall_uk) * 100)
mae_garch_rainfall_uk <- mean(abs(data_aktual_rainfall_uk - prediksi_rainfal_uk))
mse_garch_rainfall_uk <- mean((data_aktual_rainfall_uk - prediksi_rainfal_uk)^2)

cat("MAPE from garch:", mape_garch_rainfall_uk, "%\n")
cat("RMSE:", rmse_garch_rainfall_uk, "\n")
cat("MAE:", mae_garch_rainfall_uk, "\n")
cat("MSE:", mse_garch_rainfall_uk, "\n")
```
MAPE from garch: 100 %
RMSE: 7.835492 
MAE: 7.745103 
...
result_table$Produktivitas_Ubi_Kayu <- format(result_table$Produktivitas_Ubi_Kayu, nsmall = 2)
result_table$Produksi_Ubi_Kayu <- format(result_table$Produksi_Ubi_Kayu, nsmall = 2)
result_table$Produktivitas_Ubi_Jalar <- format(result_table$Produktivitas_Ubi_Jalar, nsmall = 2)
result_table$Produksi_Ubi_Jalar <- format(result_table$Produksi_Ubi_Jalar, nsmall = 2)
result_table$Produksi_jagung <- format(result_table$Produksi_jagung, nsmall = 2)
result_table$Produktivitas_jagung <- format(result_table$Produktivitas_jagung, nsmall = 2)
result_table$Produksi_Kedelai <- format(result_table$Produksi_Kedelai, nsmall = 2)
result_table$Produktivitas_kedelai <- format(result_table$Produktivitas_kedelai, nsmall = 2)
result_table$Produksi_kacang_tanah <- format(result_table$Produksi_kacang_tanah, nsmall = 2)
result_table$Produktivitas_kacang_tanah <- format(result_table$Produktivitas_kacang_tanah, nsmall = 2)
result_table$Produksi_kacang_hijau <- format(result_table$Produksi_kacang_hijau, nsmall = 2)
result_table$Produktivitas_kacang_hijau <- format(result_table$Produktivitas_kacang_hijau, nsmall = 2)
```
...
result_table <- data.frame(
  Metric = character(),
  Produksi_jagung =numeric(),
  Produktivitas_jagung =numeric(),
  Produksi_Kedelai =numeric(),
  Produktivitas_kedelai =numeric(),
  Produksi_kacang_tanah =numeric(),
  Produktivitas_kacang_tanah =numeric(),
  Produksi_kacang_hijau =numeric(),
  Produktivitas_kacang_hijau =numeric(),
  Produktivitas_Ubi_Kayu = numeric(),
  Produksi_Ubi_Kayu = numeric(),
  Produktivitas_Ubi_Jalar = numeric(),
  Produksi_Ubi_Jalar = numeric()
)

# Menambahkan hasil evaluasi ke tabel
result_table <- rbind(
  result_table,
  data.frame(
    Metric = "MAPE",
    Produktivitas_Ubi_Kayu = mape_garch_ptuk,
    Produksi_Ubi_Kayu = mape_garch_uk,
    Produktivitas_Ubi_Jalar = mape_garch_ptuj,
    Produksi_Ubi_Jalar = mape_garch_uj,
    Produksi_jagung = mape_garch_j,
    Produktivitas_jagung = mape_garch_ptj,
    Produksi_Kedelai =mape_garch_kk,
    Produktivitas_kedelai =mape_garch_ptkk,
    Produksi_kacang_tanah =mape_garch_kt,
    Produktivitas_kacang_tanah =mape_garch_ptkt,
    Produksi_kacang_hijau =mape_garch_kh,
    Produktivitas_kacang_hijau =mape_garch_ptkh
    
  ),
  data.frame(
    Metric = "RMSE",
    Produktivitas_Ubi_Kayu = rmse_garch_ptuk,
    Produksi_Ubi_Kayu = rmse_garch_uk,
    Produktivitas_Ubi_Jalar = rmse_garch_ptuj,
    Produksi_Ubi_Jalar = rmse_garch_uj,
    Produksi_jagung = rmse_garch_j,
    Produktivitas_jagung = rmse_garch_ptj,
    Produksi_Kedelai =rmse_garch_kk,
    Produktivitas_kedelai =rmse_garch_ptkk,
    Produksi_kacang_tanah =rmse_garch_kt,
    Produktivitas_kacang_tanah =rmse_garch_ptkt,
    Produksi_kacang_hijau =rmse_garch_kh,
    Produktivitas_kacang_hijau =rmse_garch_ptkh
  ),
  data.frame(
    Metric = "MAE",
    Produktivitas_Ubi_Kayu = mae_garch_ptuk,
    Produksi_Ubi_Kayu = mae_garch_uk,
    Produktivitas_Ubi_Jalar = mae_garch_ptuj,
    Produksi_Ubi_Jalar = mae_garch_uj,
    Produksi_jagung = mae_garch_j,
    Produktivitas_jagung = mae_garch_ptj,
    Produksi_Kedelai =mae_garch_kk,
    Produktivitas_kedelai =mae_garch_ptkk,
    Produksi_kacang_tanah =mae_garch_kt,
    Produktivitas_kacang_tanah =mae_garch_ptkt,
    Produksi_kacang_hijau =mae_garch_kh,
    Produktivitas_kacang_hijau =mae_garch_ptkh
  ),
  data.frame(
    Metric = "MSE",
    Produktivitas_Ubi_Kayu = mse_garch_ptuk,
    Produksi_Ubi_Kayu = mse_garch_uk,
    Produktivitas_Ubi_Jalar = mse_garch_ptuj,
    Produksi_Ubi_Jalar = mse_garch_uj,
    Produksi_jagung = mse_garch_j,
    Produktivitas_jagung =mse_garch_ptj,
    Produksi_Kedelai =mse_garch_kk,
    Produktivitas_kedelai =mse_garch_ptkk,
    Produksi_kacang_tanah =mse_garch_kt,
    Produktivitas_kacang_tanah =mse_garch_ptkt,
    Produksi_kacang_hijau =mse_garch_kh,
    Produktivitas_kacang_hijau =mse_garch_ptkh
