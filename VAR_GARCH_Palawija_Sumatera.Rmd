...
avg_rainfall.l1  -3.674e+04  1.162e+04  -3.161  0.00176 ** 
avg_rainfall.l4   2.745e+04  1.207e+04   2.275  0.02375 * 
F-statistic: 115.1 on 16 and 258 DF,  p-value: < 2.2e-16 

Estimation results for equation avg_rainfall:
Produktivitas.l4  3.193e-02  1.174e-02   2.720  0.00697 **
Produktivitas.l5 -4.793e-02  9.219e-03  -5.199 4.08e-07 ***
F-statistic:  63.7 on 16 and 258 DF,  p-value: < 2.2e-16 

Estimation results for equation Produktivitas: 
avg_rainfall.l1  -1.945e+00  4.242e-01  -4.586 7.05e-06 ***
avg_rainfall.l4   2.014e+00  4.404e-01   4.574 7.44e-06 ***
F-statistic:  63.7 on 16 and 258 DF,  p-value: < 2.2e-16 
...
```
Granger :p-value = 9.94e-08
Instant : p-value = 0.1456
berdarkan p-value Uji Grager dan isntast Jadi dapat disimpulkan bahwa terdapat bukti statisktik bahwa curah hujan memiilki hubungan sebab akibat jangka penjang dengan Produktivitas.
...
```
Hasil:
avg_rainfall p-valuenya adalah 0.398. 
nilai intercept adalah 166846  
f-statistik 0.7161 p-valuenya adalah 0.3982
Multiple R-squared:  0.002569
...
*Intercept:*   15.3966 
*avg_rainfall:* koefisien: 2.8064 dan p-valuenya: 0.000882 ***
*F-statistic:*  11.3   p-value: 0.0008824
...
residuals_var <- residuals(var_model_jagung_Sumatera)
```

```{r}
# Menghitung MAPE untuk ketiga variabel
mape_produksi <- mean(abs(residuals_var[, "Produksi"] / Sumatera_jagung$Produksi)) * 100
mape_avg_rainfall <- mean(abs(residuals_var[, "avg_rainfall"] / Sumatera_jagung$avg_rainfall)) * 100
mape_produktivitas <- mean(abs(residuals_var[, "Produktivitas"] / Sumatera_jagung$Produktivitas)) * 100

# Menampilkan hasil evaluasi untuk ketiga variabel
cat(paste("MAPE Produksi: ", mape_produksi, "%\n"))
cat(paste("MAPE Avg Rainfall: ", mape_avg_rainfall, "%\n"))
cat(paste("MAPE Produktivitas: ", mape_produktivitas, "%\n"))

```


```{r}
# Menghitung MSE, MAE, MAPE, dan RMSE
mse_value <- mean(residuals_var^2)
mae_value <- mean(abs(residuals_var))
...
cat(paste("MSE: ", mse_value, "\n"))
cat(paste("MAE: ", mae_value, "\n"))
...
### Membuat Prediksi Jagung Sumatera dengan Model VAR
```{r}
# Prediksi 10 periode ke depan
forecast_jagung_Sumatera <- predict(var_model_jagung_Sumatera, n.ahead = 10)
forecast_jagung_Sumatera
```
```{r}
# Mengambil hanya kolom "fcst" dari setiap variabel
forecast_results_fcst <- lapply(forecast_jagung_Sumatera$fcst, function(x) x[, "fcst"])

# Menampilkan hasil
print(forecast_results_fcst)
```


```{r}
# Membuat data frame untuk hasil prediksi
predicted_data <- as.data.frame(forecast_results_fcst)
predicted_data
```

```{r}
# Menambahkan variabel tahun 2023 sampai 2033
predicted_data$Tahun <- seq(2023, 2032)
predicted_data
```

```{r}
# Visualisasi tren produksi jagung
plot_tren_produksi <- ggplot(predicted_data, aes(x = Tahun, y = Produksi, color = "Prediksi")) +
  geom_line() +
  geom_point() +
...
```{r}
# Visualisasi tren produktivias jagung
plot_tren_produktivitas <- ggplot(predicted_data, aes(x = Tahun, y = Produktivitas, color = "Prediksi")) +
  geom_line() +
  geom_point() +
...
# Menampilkan plot
print(plot_tren_produktivitas)
```

```{r}
# Visualisasi tren Curah hujan sumatera
plot_avgrainfall <- ggplot(predicted_data, aes(x = Tahun, y = avg_rainfall, color = "Prediksi")) +
  geom_line() +
  geom_point() +
...
# Menampilkan plot
print(plot_avgrainfall)
```

### Menguji kinerja model
```{r}
# Mengambil data aktual untuk periode yang sama
actual_data <- Sumatera_jagung %>%
  filter(Tahun >= 2023 & Tahun <= 2032)
actual_data
```

```{r}
# Pilih variabel yang akan diprediks
prediksi_jagung_Sumatera <- predicted_data[, c("Produksi", "avg_rainfall", "Produktivitas")]
prediksi_jagung_Sumatera
```
```{r}
mae_Jagung <- mae(ts_data_jagung_Sumatera[,1], predicted_data[,1])
mae_Jagung
```
```{r}
mae_Jagung<- mae(ts_data_jagung_Sumatera[,1], prediksi_jagung_Sumatera[,1])
mae_Jagung
```
...
actual_values_jagung_Sumatera <- ts_data_jagung_Sumatera[(nrow(ts_data_jagung_Sumatera) - best_lag_jagung_Sumatera$selection[1] + 1):nrow(ts_data_jagung_Sumatera), "Produksi"]
actual_values_jagung_Sumatera
```

```{r}
# Menghitung metrik evaluasi (RMSE, MAPE, MAE, MSE)
rmse_jagung_Sumatera1 <- sqrt(mean((data_var_jagung - prediksi_jagung_Sumatera)^2))
mape_jagung_Sumatera1 <- mean(abs((data_var_jagung- prediksi_jagung_Sumatera) / data_var_jagung)) * 100
mae_jagung_Sumatera1 <- mean(abs(data_var_jagung - prediksi_jagung_Sumatera))
mse_jagung_Sumatera1 <- mean((data_for_var - prediksi_jagung_Sumatera)^2)
```
```{r}
# Menampilkan hasil evaluasi
print(paste("RMSE Jagung Sumatera: ", rmse_jagung_Sumatera1))
print(paste("MAPE Jagung Sumatera: ", mape_jagung_Sumatera1))
print(paste("MAE Jagung Sumatera: ", mae_jagung_Sumatera1))
print(paste("MSE Jagung Sumatera: ", mse_jagung_Sumatera1))
```
```{r}

# Menghitung error measures
rmse_1 <- sqrt(mean(data_var_jagung$prediksi_jagung_Sumatera^2))  # RMSE
mae_1 <- mean(abs(data_var_jagung$prediksi_jagung_Sumatera))  # MAE
mape_1 <- mean(abs(data_var_jagung$E1 / var_pred$fcst$mean)) * 100  # MAPE
mse_1 <- mean(data_var_jagung$E1^2)  # MSE
```
...
```{r}
#Prediksi dan menghitung kesalahan prediksi
forecastVar <- predicted_data$mean
observedVar <- as.data.frame(data_var_jagung[, -ncol(data_var_jagung), ncol(data_var_jagung):1])
```


```{r}
predictedVar <- as.matrix(forecastVar[, ncol(forecastVar):1])
actualVar <- as.matrix(observedVar[, ncol(observedVar):1])
```


```{r}
# Menghitung RMSE, MAE, MAPE, MSE
RMSE_var <- rmse(predictedVar, actualVar)
MAE_var <- mae(predictedVar, actualVar)
MAPE_var <- mape(predictedVar, actualVar)
MSE_var <- mse(predictedVar, actualVar)

print(paste("RMSE: ", RMSE_var))
print(paste("MAE: ", MAE_var))
print(paste("MAPE: ", MAPE_var))
print(paste("MSE: ", MSE_var))
```

```{r}
# Merubah type data karakter menjadi numeric
prediksi_jagung_Sumatera$Produksi <- as.numeric(prediksi_jagung_Sumatera$Produksi)
prediksi_jagung_Sumatera$Produktivitas <- as.numeric(prediksi_jagung_Sumatera$Produktivitas)
prediksi_jagung_Sumatera$avg_rainfall<- as.numeric(prediksi_jagung_Sumatera$avg_rainfall)
prediksi_jagung_Sumatera
```

```{r}
# Pastikan kedua variabel berupa numerik
stopifnot(is.numeric(data_var_jagung$Produksi), is.numeric(prediksi_jagung_Sumatera$Produksi))
```
```{r}
# Pastikan semua kolom dalam data frame berupa numerik
stopifnot(all(sapply(prediksi_jagung_Sumatera, is.numeric)))
```
...
# Hitung MAE
mae_value_var <- mae(data_var_jagung$Produksi, prediksi_jagung_Sumatera$Produksi)

# Hitung MSE
mse_value_var <- mse(data_var_jagung$Produksi, prediksi_jagung_Sumatera$Produksi)

# Hitung RMSE
rmse_value_var <- rmse(data_var_jagung$Produksi, prediksi_jagung_Sumatera$Produksi)

# Hitung MAPE
mape_value_var <- mape(data_var_jagung$Produksi, prediksi_jagung_Sumatera$Produksi)

# Tampilkan hasil evaluasi
cat(paste("MAE: ", mae_value_var, "\n"))
cat(paste("MSE: ", mse_value_var, "\n"))
cat(paste("RMSE: ", rmse_value_var, "\n"))
cat(paste("MAPE: ", mape_value_var, "%\n"))
```
...
```{r}
# Hitung MAE
mae_value_var2 <- mae(data_var_jagung$Produksi, prediksi_jagung_Sumatera$Produksi)

# Hitung MSE
mse_value_var2 <- mse(data_var_jagung$Produksi, prediksi_jagung_Sumatera$Produksi)

# Hitung RMSE
rmse_value_var2 <- rmse(data_var_jagung$Produksi, prediksi_jagung_Sumatera$Produksi)

# Hitung MAPE
mape_value_var2 <- mape(data_var_jagung$Produksi, prediksi_jagung_Sumatera$Produksi)

# Tampilkan hasil evaluasi
cat(paste("MAE: ", mae_value_var2, "\n"))
cat(paste("MSE: ", mse_value_var2, "\n"))
cat(paste("RMSE: ", rmse_value_var2, "\n"))
cat(paste("MAPE: ", mape_value_var2, "%\n"))
```
...
```
Estimation results for equation Produksi: 
Produktivitas.l3  1.133e+03  4.960e+02   2.283  0.02329 *
Multiple R-Squared: 0.6857,	
F-statistic: 32.99   p-value: < 2.2e-16 


Estimation results for equation avg_rainfall: 
Produktivitas.l1  7.732e-02  3.726e-02   2.075 0.039016 *
Produksi.l4      -1.369e-05  6.630e-06  -2.065 0.039980 *  
avg_rainfall.l4   1.732e-01  6.236e-02   2.777 0.005915 **
Produktivitas.l5 -1.423e-01  3.986e-02  -3.569 0.000432 ***
Multiple R-Squared: 0.1746,	 
F-statistic: 3.198 ,  p-value: 5.224e-05 


Estimation results for equation Produkivitas: 
Produktivitas.l1  5.199e-01  6.532e-02   7.960 6.66e-14 ***
Multiple R-Squared: 0.3855,	
F-statistic: 9.487,  p-value: < 2.2e-16 
...
#Uji Granger Causality digunakan untuk menguji apakah suatu variabel dapat digunakan untuk memprediksi variabel lain dalam model VAR
```
Granger= p-value = 0.4478 dan  instant = p-value = 0.07575
*kesimpulan*
...
avg_rainfall   0.2434     0.1237   1.968   0.0501 . 
F-statistic: 3.873 ,  p-value: 0.05013
Multiple R-squared:  0.01457
*Kesimpulan*
Terdapat bukti yang cukup (p-value sekitar 0.0501) bahwa curah hujan (avg_rainfall) mungkin memiliki dampak pada Produktivitas. Namun, R-squared yang rendah menunjukkan bahwa model mungkin tidak memberikan penjelasan yang kuat
...
(Intercept)     46518      11764   3.954 9.88e-05 ***
avg_rainfall    -4114       1513  -2.720  0.00697 **
Multiple R-squared:  0.02746
F-statistic: 7.398  p-value: 0.006966
*Kesimpulan*
Terdapat bukti yang cukup (p-value sekitar 0.00697) bahwa curah hujan (avg_rainfall) memiliki dampak yang signifikan pada Produksi. Namun, R-squared yang rendah menunjukkan bahwa model mungkin tidak memberikan penjelasan yang kuat.
...
### Membuat Prediksi Kacang Hijau Sumatera dengan Model VAR
```{r}
# Prediksi 10 periode ke depan
forecast_Kedelai <- predict(var_model_kedelai_Sumatera, n.ahead = 10)
forecast_Kedelai
```
```{r}
# Mengambil hanya kolom "fcst" dari setiap variabel
forecast_results_kedelai <- lapply(forecast_Kedelai$fcst, function(x) x[, "fcst"])

# Menampilkan hasil
print(forecast_results_kedelai)
```
```{r}
# Membuat data frame untuk hasil prediksi
predicted_data_kedelai <- as.data.frame(forecast_results_kedelai)
predicted_data_kedelai
```
```{r}
# Menambahkan variabel tahun 2023 sampai 2033
predicted_data_kedelai$Tahun <- seq(2023, 2032)
predicted_data_kedelai
```

```{r}
# Visualisasi tren produksi Kacang kedelai
plot_produksi_kedelai <- ggplot(predicted_data_kedelai, aes(x = Tahun, y = Produksi, color = "Prediksi")) +
  geom_line() +
  geom_point() +
  labs(title = "Prediksi Tren Produksi Kacang kedelai Sumatera (2023-2033)",
       x = "Tahun",
       y = "Produksi Kacang kedelai",
       color = "Metode Prediksi") +
  scale_y_continuous(labels = scales::comma) +
  theme_minimal() +
  theme(legend.position = "right")
...
# Menampilkan plot
print(plot_produksi_kedelai)
```

```{r}
# Visualisasi tren produksi Kacang kedelai
plot_produktivitas_kedelai <- ggplot(predicted_data_kedelai, aes(x = Tahun, y = Produktivitas, color = "Prediksi")) +
  geom_line() +
  geom_point() +
  labs(title = "Prediksi Tren Produktivitas Kacang Kedelai Sumatera (2023-2033)",
       x = "Tahun",
       y = "Produktivitas Jagung",
       color = "Metode Prediksi") +
  scale_y_continuous(labels = scales::comma) +
...
# Menampilkan plot
print(plot_produktivitas_kedelai)
```

```{r}
# Visualisasi tren Curah hujan Sumatera
plot_avgrainfall_Kh <- ggplot(predicted_data_kedelai, aes(x = Tahun, y = avg_rainfall, color = "Prediksi")) +
  geom_line() +
  geom_point() +
  labs(title = "Prediksi curah hujan Sumatera (2023-2033)",
       x = "Tahun",
...
Estimation results for equation Produksi: 
avg_rainfall.l4  -143.02969   71.42177  -2.003  0.04660 *
F-statistic: 50.22 on 16 and 195 DF,  p-value: < 2.2e-16 


Estimation results for equation avg_raifall: 
Produktivitas.l4  2.141e-01  4.567e-02   4.687 5.19e-06 ***
F-statistic:  3.52 on 16 and 195 DF,  p-value: 1.474e-05 

Estimation results for equation Produktivitas: 
avg_rainfall.l4   3.388e-01  1.292e-01   2.622 0.009439 **
avg_rainfall.l5   2.879e-01  1.308e-01   2.202 0.028834 * 
F-statistic: 8.312 on 16 and 195 DF,  p-value: 4.24e-15 
...
```
avg_rainfall  setimate: -274.4    p-value: 0.051246 
Multiple R-squared:  0.01756
F-statistic: 3.843,  p-value: 0.05125
*Kesimpulan* terdapat bukti bahwa curah tidak mempergaruhi mempergaruhi produksi kacang hijau
...
```
 Estimate Std. Error t value Pr(>|t|)    
(Intercept)    6.5065     1.1116   5.853 1.78e-08 ***
avg_rainfall   0.4755     0.1411   3.369 0.000894 ***
Multiple R-squared:  0.05014,	Adjusted R-squared:  0.04572 
F-statistic: 11.35 on 1 and 215 DF,  p-value: 0.0008943
...
```{r}
# Prediksi 10 periode ke depan
forecast_Kacang_Hijau <- predict(var_model_kacanghijau_Sumatera, n.ahead = 10)
forecast_Kacang_Hijau
```
```{r}
# Mengambil hanya kolom "fcst" dari setiap variabel
forecast_results_Kacanghijau <- lapply(forecast_Kacang_Hijau$fcst, function(x) x[, "fcst"])

# Menampilkan hasil
print(forecast_results_Kacanghijau)
```
```{r}
# Membuat data frame untuk hasil prediksi
predicted_data_Kacanghijau <- as.data.frame(forecast_results_Kacanghijau)
predicted_data_Kacanghijau
```
```{r}
# Menambahkan variabel tahun 2023 sampai 2033
predicted_data_Kacanghijau$Tahun <- seq(2023, 2032)
predicted_data_Kacanghijau
```

```{r}
# Visualisasi tren produksi kacang hijau
plot_produksi_Kacanghijau <- ggplot(predicted_data_Kacanghijau, aes(x = Tahun, y = Produksi, color = "Prediksi")) +
  geom_line() +
  geom_point() +
  labs(title = "Prediksi Tren Produksi Kacang Hijau Sumatera (2023-2033)",
       x = "Tahun",
...
# Menampilkan plot
print(plot_produksi_Kacanghijau)
```

```{r}
# Visualisasi tren produksi kacang hijau
plot_produktivitas_Kacanghijau <- ggplot(predicted_data_Kacanghijau, aes(x = Tahun, y = Produktivitas, color = "Prediksi")) +
  geom_line() +
  geom_point() +
  labs(title = "Prediksi Tren Produktivitas Kacang Hijau Sumatera (2023-2033)",
       x = "Tahun",
       y = "Produktivitas Jagung",
       color = "Metode Prediksi") +
  scale_y_continuous(labels = scales::comma) +
...
# Menampilkan plot
print(plot_produktivitas_Kacanghijau)
```

```{r}
# Visualisasi tren Curah hujan Sumatera
plot_avgrainfall_Kh <- ggplot(predicted_data_Kacanghijau, aes(x = Tahun, y = avg_rainfall, color = "Prediksi")) +
  geom_line() +
  geom_point() +
  labs(title = "Prediksi curah hujan Sumatera (2023-2033)",
       x = "Tahun",
...
Estimation results for equation Produksi: 
avg_rainfall.l1   368.63397  153.31163   2.404 0.016904 *
avg_rainfall.l4  -504.09274  149.84054  -3.364 0.000885 ***
Multiple R-Squared: 0.8454, F-statistic: 87.82 on 16 and 257 DF,  p-value: < 2.2e-16  

Estimation results for equation avg_rainfall: 
Produksi.l1      -6.662e-05  2.474e-05  -2.692  0.00756 ** 
Produktivitas.l3  1.770e-01  5.960e-02   2.969  0.00327 ** 
Produksi.l4      -7.044e-05  2.895e-05  -2.434  0.01563 *  
Produksi.l5       6.455e-05  2.245e-05   2.875  0.00437 ** 
Multiple R-Squared: 0.2155, F-statistic: 4.411 on 16 and 257 DF,  p-value: 1.086e-07

Estimation results for equation Produktivitas: 
avg_rainfall.l2  -1.734e-01  7.489e-02  -2.315  0.02138 * 
Multiple R-Squared: 0.6028, F-statistic: 24.38 on 16 and 257 DF,  p-value: < 2.2e-16 
...
#Uji Granger Causality digunakan untuk menguji apakah suatu variabel dapat digunakan untuk memprediksi variabel lain dalam model VAR
```
Granger p-value = 0.004093
Instant p-value = 0.9764
...
#Uji Granger Causality digunakan untuk menguji apakah suatu variabel dapat digunakan untuk memprediksi variabel lain dalam model VAR
```
Granger p-value = 0.007193
Instant p-value =0.08421
*Kesimpulan* Terdapat pengaruh jangka panjang produksi terhadap Porduktivitas dan avg_rainfall
...
Coefficients:
             Estimate Std. Error t value Pr(>|t|)    
(Intercept)   10645.5     2810.6   3.788 0.000187 ***
avg_rainfall   -513.0      361.1  -1.420 0.156629 
Multiple R-squared:  0.00723,	Adjusted R-squared:  0.003646 
F-statistic: 2.017 on 1 and 277 DF,  p-value: 0.1566
...
```
 Estimate Std. Error t value Pr(>|t|)    
(Intercept)    9.7845     0.7911   12.37   <2e-16 ***
avg_rainfall   0.2256     0.1017    2.22   0.0272 *  
Multiple R-squared:  0.01748,	Adjusted R-squared:  0.01393 
F-statistic: 4.928 on 1 and 277 DF,  p-value: 0.02724
*Kesimpulan* Terdapat pengaruh curah hujan terhadap produktivitas kacang tanah.
...
### Membuat Prediksi Kacang tanah Sumatera dengan Model VAR
```{r}
# Prediksi 10 periode ke depan
forecast_Kacang_tanah <- predict(var_model_kacangtanah_Sumatera, n.ahead = 10)
forecast_Kacang_tanah
```
```{r}
# Mengambil hanya kolom "fcst" dari setiap variabel
forecast_results_Kacangtanah <- lapply(forecast_Kacang_tanah$fcst, function(x) x[, "fcst"])

# Menampilkan hasil
print(forecast_results_Kacangtanah)
```
```{r}
# Membuat data frame untuk hasil prediksi
predicted_data_Kacangtanah <- as.data.frame(forecast_results_Kacangtanah)
predicted_data_Kacangtanah
```
```{r}
# Menambahkan variabel tahun 2023 sampai 2033
predicted_data_Kacangtanah$Tahun <- seq(2023, 2032)
predicted_data_Kacangtanah
```

```{r}
# Visualisasi tren produksi kacang tanah
plot_produksi_Kacangtanah <- ggplot(predicted_data_Kacangtanah, aes(x = Tahun, y = Produksi, color = "Prediksi")) +
  geom_line() +
  geom_point() +
...
# Menampilkan plot
print(plot_produksi_Kacangtanah)
```

```{r}
# Visualisasi tren produksi kacang tanah
plot_produktivitas_Kacangtanah <- ggplot(predicted_data_Kacangtanah, aes(x = Tahun, y = Produktivitas, color = "Prediksi")) +
  geom_line() +
  geom_point() +
...
# Menampilkan plot
print(plot_produktivitas_Kacangtanah)
```

```{r}
# Visualisasi tren Curah hujan Sumatera
plot_avgrainfall_Kt <- ggplot(predicted_data_Kacangtanah, aes(x = Tahun, y = avg_rainfall, color = "Prediksi")) +
  geom_line() +
  geom_point() +
  labs(title = "Prediksi curah hujan Sumatera (2023-2033)",
       x = "Tahun",
...
```
Estimation results for equation Produksi: 
avg_rainfall.l1   1.814e+03  8.695e+02   2.086   0.0380 * 
avg_rainfall.l5  -1.897e+03  8.822e+02  -2.151   0.0324 *
F-statistik p-value: < 2.2e-16 

Estimation results for equation avg_rainfall: 
Produksi.l1      -1.413e-05  4.270e-06  -3.309 0.001070 **
Produksi.l2       1.830e-05  5.834e-06   3.136 0.001908 ** 
Produktivitas.l3 -6.053e-03  2.781e-03  -2.176 0.030453 *  
Produktivitas.l4  1.063e-02  2.784e-03   3.818 0.000169 ***
Produksi.l5       1.097e-05  4.361e-06   2.516 0.012485 *  
Produktivitas.l5 -8.982e-03  1.994e-03  -4.505 1.01e-05 ***
F-statistik p-value:2.831e-08  

Estimation results for equation produktivitas: 
avg_rainfall.l1  -4.457e+00  1.832e+00  -2.433 0.015670 * 
avg_rainfall.l4   6.654e+00  1.825e+00   3.645 0.000323 ***
F-statistik p-value: < 2.2e-16 
...
#Uji Granger Causality digunakan untuk menguji apakah suatu variabel dapat digunakan untuk memprediksi variabel lain dalam model VAR
```
Granger p-value = 0.0003156
Instant  p-value = 0.7472
...
#Uji Granger Causality digunakan untuk menguji apakah suatu variabel dapat digunakan untuk memprediksi variabel lain dalam model VAR
```
Granger p-value =0.0006602 dan Instant p-value  0.02525
terdapat bukti signifikan sebab akibat produksi terhadap avg_rainfall dan produksivitas baik jangkah panjang dan di jangka instant
...
Coefficients:
             Estimate Std. Error t value Pr(>|t|)   
(Intercept)    49.338     27.533   1.792   0.0742 . 
avg_rainfall    9.460      3.536   2.676   0.0079 **
...
### Membuat Prediksi Ubi jalar Sumatera dengan Model VAR
```{r}
# Prediksi 10 periode ke depan
forecast_ubijalar <- predict(var_model_ubijalar_Sumatera, n.ahead = 10)
forecast_ubijalar
```
```{r}
# Mengambil hanya kolom "fcst" dari setiap variabel
forecast_results_ubijalar <- lapply(forecast_ubijalar$fcst, function(x) x[, "fcst"])

# Menampilkan hasil
print(forecast_results_ubijalar)
```
```{r}
# Membuat data frame untuk hasil prediksi
predicted_data_ubijalar <- as.data.frame(forecast_results_ubijalar)
predicted_data_ubijalar
```
```{r}
# Menambahkan variabel tahun 2023 sampai 2033
predicted_data_ubijalar$Tahun <- seq(2023, 2032)
predicted_data_ubijalar
```

```{r}
# Visualisasi tren produksi Ubi jalar
plot_produksi_ubijalar <- ggplot(predicted_data_ubijalar, aes(x = Tahun, y = Produksi, color = "Prediksi")) +
  geom_line() +
  geom_point() +
  labs(title = "Prediksi Tren Produksi Ubi jalar Sumatera (2023-2033)",
       x = "Tahun",
       y = "Produksi Ubi jalar",
       color = "Metode Prediksi") +
  scale_y_continuous(labels = scales::comma) +
  theme_minimal() +
  theme(legend.position = "right")
...
# Menampilkan plot
print(plot_produksi_ubijalar)
```

```{r}
# Visualisasi tren produksi Ubi jalar
plot_produktivitas_ubijalar <- ggplot(predicted_data_ubijalar, aes(x = Tahun, y = Produktivitas, color = "Prediksi")) +
  geom_line() +
  geom_point() +
  labs(title = "Prediksi Tren Produktivitas Ubi jalar Sumatera (2023-2033)",
       x = "Tahun",
       y = "Produktivitas Jagung",
       color = "Metode Prediksi") +
  scale_y_continuous(labels = scales::comma) +
...
# Menampilkan plot
print(plot_produktivitas_ubijalar)
```

```{r}
# Visualisasi tren Curah hujan Sumatera
plot_avgrainfall_uj <- ggplot(predicted_data_ubijalar, aes(x = Tahun, y = avg_rainfall, color = "Prediksi")) +
  geom_line() +
  geom_point() +
  labs(title = "Prediksi curah hujan Sumatera (2023-2033)",
       x = "Tahun",
...
Estimation results for equation avg_rainfall: 
Produktivitas.l1  4.090e-03  1.371e-03   2.982  0.00314 ** 
Produksi.l2       4.111e-07  1.843e-07   2.231  0.02657 *  
Produktivitas.l2 -4.196e-03  1.782e-03  -2.355  0.01927 *  
Produktivitas.l4  4.892e-03  1.773e-03   2.760  0.00620 ** 
Produktivitas.l5 -6.183e-03  1.388e-03  -4.454 1.26e-05 ***
F-statistic: 4.292 on 16 and 258 DF,  p-value: 1.973e-07 
 
Estimation results for equation Produktivitas: 
avg_rainfall.l1  -1.274e+01  2.695e+00  -4.727 3.75e-06 ***
avg_rainfall.l4   9.707e+00  2.726e+00   3.560 0.000441 ***
avg_rainfall.l5   6.822e+00  2.780e+00   2.454 0.014797 * 
p-value: < 2.2e-16  
...
#Uji Granger Causality digunakan untuk menguji apakah suatu variabel dapat digunakan untuk memprediksi variabel lain dalam model VAR
```
Granger = p-value = 0.6661
Instant = p-value = 0.5903
...
Coefficients:
             Estimate Std. Error t value Pr(>|t|)  
(Intercept)    89.982     40.405   2.227   0.0267 *
avg_rainfall   13.375      5.188   2.578   0.0105 *
...
### Membuat Prediksi Ubi kayu Sumatera dengan Model VAR
```{r}
# Prediksi 10 periode ke depan
...
Estimation results for equation produktivitas: 
avg_rainfall.l1  -1.371e+00  2.921e-01  -4.693 0.001555 **
avg_rainfall.l3   6.605e-01  2.427e-01   2.722 0.026188 * 
Produksi.l4       4.916e-06  2.039e-06   2.411 0.042429 *
F-statistik p-value:6.623e-10 
...
#Uji Granger Causality digunakan untuk menguji apakah suatu variabel dapat digunakan untuk memprediksi variabel lain dalam model VAR
```
Granger p-value = 0.0005302
Instant  p-value = 0.49
terdapat bukti signifikan sebab akibat avg-rainfall terhadap produksi dan produksivitas dalam jangkah panjang tetapi tidak di jangka instant
...
```
Granger p-value =1.346e-09 dan Instant p-value 0.6807
terdapat bukti signifikan sebab akibat produksi terhadap avg_rainfall dan produksivitas jangkah panjang dan tidak jangka instant
...
#Uji Granger Causality digunakan untuk menguji apakah suatu variabel dapat digunakan untuk memprediksi variabel lain dalam model VAR
```
Granger p-value = 0.419
Instant  p-value = 0.1069
terdapat bukti signifikan sebab akibat avg-rainfall tidak terhadap pada produksi dan produksivitas dalam jangkah panjang dan jangka instant
...
```
Granger p-value =0.7341 dan Instant p-value  0.1094
tidak terdapat bukti signifikan sebab akibat produksi terhadap avg_rainfall dan produksivitas jangkah panjang dan jangka instant
...
```
  Estimate Std. Error t value Pr(>|t|)   
(Intercept)    9.6869     3.4544   2.804  0.00906 **
avg_rainfall   0.2407     0.4407   0.546  0.58929              
...
```
Estimate Std. Error t value Pr(>|t|)   
(Intercept)     82440      24214   3.405  0.00202 **
avg_rainfall    -8405       3089  -2.721  0.01107 * 
...
Estimation results for equation produktivitas: 
avg_rainfall.l1  -3.933e+00  1.329e+00  -2.959   0.0416 *
F-statistik p-value:0.03056 
...
#Uji Granger Causality digunakan untuk menguji apakah suatu variabel dapat digunakan untuk memprediksi variabel lain dalam model VAR
```
Granger p-value = 0.01785
Instant  p-value =  0.005492
terdapat bukti signifikan sebab akibat avg-rainfall terhadap produksi dan produksivitas dalam jangkah panjang dan di jangka instant
...
```
Granger p-value =0.01102 dan Instant p-value 0.005521
terdapat bukti signifikan sebab akibat produksi terhadap avg_rainfall dan produksivitas jangkah panjang dan dan jangka instant
...
```
Estimate Std. Error t value Pr(>|t|)
(Intercept)    3.3811     4.6463   0.728    0.474
avg_rainfall   0.8274     0.5870   1.410    0.171
...
```
Estimate Std. Error t value Pr(>|t|)  
(Intercept)   16928.7     6779.7   2.497   0.0198 *
avg_rainfall  -1446.0      856.5  -1.688   0.1043  
...
#Uji Granger Causality digunakan untuk menguji apakah suatu variabel dapat digunakan untuk memprediksi variabel lain dalam model VAR
```
Granger p-value = 0.4299
Instant  p-value = 0.1633
tidak terdapat bukti signifikan sebab akibat avg-rainfall terhadap produksi dan produksivitas dalam jangkah panjang dan di jangka instant
...
```
Granger p-value =0.8945 an Instant p-value 0.06302
tidak terdapat bukti signifikan sebab akibat produksi terhadap avg_rainfall dan produksivitas jangkah panjang dan tidak jangka instant
...
```
             Estimate Std. Error t value Pr(>|t|)    
(Intercept)    8.2549     1.5714   5.253 1.39e-05 ***
avg_rainfall   0.3856     0.2005   1.923   0.0647 
...
```
  Estimate Std. Error t value Pr(>|t|)   
(Intercept)     47107      14712   3.202  0.00339 **
avg_rainfall    -3830       1877  -2.040  0.05086 . .
...
#Uji Granger Causality digunakan untuk menguji apakah suatu variabel dapat digunakan untuk memprediksi variabel lain dalam model VAR
```
Granger p-value = 0.2464
Instant  p-value = 0.2937
tidak terdapat bukti signifikan sebab akibat avg-rainfall terhadap produksi dan produksivitas dalam jangkah panjang dan tidak di jangka instant
...
#Uji Granger Causality digunakan untuk menguji apakah suatu variabel dapat digunakan untuk memprediksi variabel lain dalam model VAR
```
Granger p-value =0.602 an Instant p-value  0.3156
tidak terdapat bukti signifikan sebab akibat produksi terhadap avg_rainfall dan produksivitas baik jangkah panjang dan di jangka instant
...
#Uji Granger Causality digunakan untuk menguji apakah suatu variabel dapat digunakan untuk memprediksi variabel lain dalam model VAR
```
Granger p-value = 0.90
Instant  p-value = 0.021
terdapat bukti signifikan sebab akibat avg-rainfall tidak terhadap produksi dan produksivitas dalam jangkah panjang tetapi signiifkan sebab- akibat di jangka instant
...
```
Granger p-value =0.8724 dan Instant p-value 0.00417
terdapat bukti signifikan sebab akibat avg-rainfall tidak terhadap produksi dan produksivitas dalam jangkah panjang tetapi signiifkan sebab- akibat di jangka instant
...
(Intercept)    -31.74     190.21  -0.167    0.869
avg_rainfall    33.31      24.27   1.372    0.181
...
```
Estimate Std. Error t value Pr(>|t|)
(Intercept)     39232     617773   0.064    0.950
avg_rainfall    94365      78817   1.197    0.241
# Interpretasi ubikayu SUMUT
Berdasarkan Model VAR, regresi dan uji grager dan isntants. Curah hujan tidak berdampak terhadap produktivitas ubikayu sumut dan tidak berdampak pada produksi Ubi Kayu sumut.


#Interpretasi UMUM 
Berdasarkan proses analsiis model VAR pada setip palawija di provinsi sumatera utara dapat di ambil kesimpulan bahwa *Avg_rainfal hanya berdampak* terhadap Produktivitas jagung.Dan curah hujan *tidak berdampak* terhadap produksi jagung Sumut, Produksi dan produktivitas ubi jalar, Produksi dan produktivitas ubi kayu, Produksi dan produktivitas kacang kedelai, Produksi dan produktivitas kacang tanah dan Produksi dan produktivitas kacang Hijau.
...
```{r}
# Menghitung metrik evaluasi (RMSE, MAPE, MAE, MSE)
rmse_jagung_Sumatera <- sqrt(mean((actual_values_jagung_Sumatera - predictions_jagung_Sumatera$fcst$Produksi)^2))
mape_jagung_Sumatera <- mean(abs((actual_values_jagung_Sumatera - predictions_jagung_Sumatera$fcst$Produksi) / actual_values_jagung_Sumatera)) * 100
mae_jagung_Sumatera <- mean(abs(actual_values_jagung_Sumatera - predictions_jagung_Sumatera$fcst$Produksi))
mse_jagung_Sumatera <- mean((actual_values_jagung_Sumatera - predictions_jagung_Sumatera$fcst$Produksi)^2)

# Menampilkan hasil evaluasi
print(paste("RMSE Jagung Sumatera: ", rmse_jagung_Sumatera))
print(paste("MAPE Jagung Sumatera: ", mape_jagung_Sumatera))
print(paste("MAE Jagung Sumatera: ", mae_jagung_Sumatera))
print(paste("MSE Jagung Sumatera: ", mse_jagung_Sumatera))
```

```{r}
# Mengambil nilai sebenarnya
actual_values_jagung_Sumatera_pt <- ts_data_jagung_Sumatera[(nrow(ts_data_jagung_Sumatera) - best_lag_jagung_Sumatera$selection[1] + 1):nrow(ts_data_jagung_Sumatera), "Produktivitas"]
actual_values_jagung_Sumatera_pt
```


```{r}
# Menghitung metrik evaluasi (RMSE, MAPE, MAE, MSE)
rmse_jagung_Sumatera_pt <- sqrt(mean((actual_values_jagung_Sumatera_pt - predictions_jagung_Sumatera$fcst$Produktivitas)^2))
mape_jagung_Sumatera_pt <- mean(abs((actual_values_jagung_Sumatera_pt - predictions_jagung_Sumatera$fcst$Produktivitas) / actual_values_jagung_Sumatera_pt)) * 100
mae_jagung_Sumatera_pt <- mean(abs(actual_values_jagung_Sumatera_pt - predictions_jagung_Sumatera$fcst$Produktivitas))
mse_jagung_Sumatera_pt <- mean((actual_values_jagung_Sumatera_pt - predictions_jagung_Sumatera$fcst$Produktivitas)^2)

# Menampilkan hasil evaluasi
print(paste("RMSE Jagung Sumatera_pt: ", rmse_jagung_Sumatera_pt))
print(paste("MAPE Jagung Sumatera_pt: ", mape_jagung_Sumatera_pt))
print(paste("MAE Jagung Sumatera_pt: ", mae_jagung_Sumatera_pt))
print(paste("MSE Jagung Sumatera_pt: ", mse_jagung_Sumatera_pt))
```


```{r}
# Mengambil nilai sebenarnya
actual_hujan <- ts_data_jagung_Sumatera[(nrow(ts_data_jagung_Sumatera) - best_lag_jagung_Sumatera$selection[1] + 1):nrow(ts_data_jagung_Sumatera), "avg_rainfall"]
actual_hujan
```
...
```{r}
# Menghitung metrik evaluasi (RMSE, MAPE, MAE, MSE)
rmse_hujan_Sumatera <- sqrt(mean((actual_hujan - predictions_jagung_Sumatera$fcst$avg_rainfall)^2))
mape_hujan_Sumatera_pt <- mean(abs((actual_hujan - predictions_jagung_Sumatera$fcst$avg_rainfall) / actual_hujan)) * 100
mae_hujan_Sumatera_pt <- mean(abs(actual_hujan - predictions_jagung_Sumatera$fcst$avg_rainfall))
mse_hujan_Sumatera_pt <- mean((actual_hujan - predictions_jagung_Sumatera$fcst$avg_rainfall)^2)

# Menampilkan hasil evaluasi
print(paste("RMSE hujan Sumatera_pt: ", rmse_hujan_Sumatera))
print(paste("MAPE hujan Sumatera_pt: ", mape_hujan_Sumatera_pt))
print(paste("MAE hujan Sumatera_pt: ", mae_hujan_Sumatera_pt))
print(paste("MSE hujan Sumatera_pt: ", mse_jagung_Sumatera_pt))
```
