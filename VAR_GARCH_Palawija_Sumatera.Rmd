...
```{r}
forecast_var_jagung_Sumatera <- predict(var_model_jagung_Sumatera, n.ahead = length(future_years))
forecast_var_jagung_Sumatera
```


```{r}
# Membuat data frame untuk hasil prediksi
predicted_data <- as.data.frame(forecast_var_jagung_Sumatera$fcst)
predicted_data
```

```{r}
# Menambahkan nama kolom sesuai dengan variabel dan tahun
colnames(predicted_data) <- c(
  paste("Produksi", future_years, sep="_")[1:4],  # Sesuaikan panjangnya
  paste("avg_rainfall", future_years, sep="_")[1:4],  # Sesuaikan panjangnya
  paste("Produktivitas", future_years, sep="_")[1:4]  # Sesuaikan panjangnya
)
predicted_data
```
...
matplot(future_years, predicted_data, type = "l", col = 1:3, lty = 1:3, xlab = "Tahun", ylab = "Nilai", main = "Prediksi Variabel")
legend("topright", legend = colnames(predicted_data), col = 1:3, lty = 1:3)

```

```{r}
# Visualisasi hasil prediksi untuk Produksi Jagung
plot(future_years, predicted_data$Produksi, type = "l", col = "blue", xlab = "Tahun", ylab = "Produksi Jagung", main = "Prediksi Produksi Jagung")
```

```{r}
# Visualisasi hasil prediksi untuk Curah Hujan
plot(future_years, predicted_data$avg_rainfall, type = "l", col = "green", xlab = "Tahun", ylab = "Curah Hujan", main = "Prediksi Curah Hujan")
```

```{r}
# Visualisasi hasil prediksi untuk Produktivitas Jagung
plot(future_years, predicted_data$Produktivitas, type = "l", col = "red", xlab = "Tahun", ylab = "Produktivitas Jagung", main = "Prediksi Produktivitas Jagung")
```

```{r}
ts_data_jagung_Sumatera2 <- as.data.frame(ts_data_jagung_Sumatera)
ts_data_jagung_Sumatera2
```


```{r}
# Contoh: Menghitung RMSE untuk Produksi Jagung
actual_data <- ts_data_jagung_Sumatera2$Produksi
rmse <- sqrt(mean((predicted_data$Produksi - actual_data)^2))
print(paste("RMSE untuk Produksi Jagung: ", rmse))
```
```{r}
class(ts_data_jagung_Sumatera)
```

```{r}
# Menghitung RMSE untuk Produksi Jagung
actual_data <- ts_data_jagung_Sumatera[, "Produksi"]
rmse <- sqrt(mean((predicted_data$Produksi - actual_data)^2))
print(paste("RMSE untuk Produksi Jagung: ", rmse))
```
```{r}
# Menetapkan properti waktu (tsp) pada actual_data
tsp(actual_data) <- tsp(ts_data_jagung_Sumatera)
```
...
```
```{r}
# Install dan memuat paket metrics
...
```{r}
# Menghitung RMSE untuk Produksi Jagung
rmse_value <- rmse(predicted_data$Produksi, actual_data)
print(paste("RMSE untuk Produksi Jagung: ", rmse_value))
...
# 5. Model  GARCH tanaman palawija

## a. GARCH terhadap produksi jagung Sumatera

```{r}
# Pilih variabel yang akan diprediksi
garch_jagung <- Sumatera_jagung[, c("Tahun" ,"Produksi", "avg_rainfall")]
garch_jagung
```

```{r}
## Penentuan Variabel Dependensi dan Independensi
dependen_produksi <- garch_jagung$Produksi
independen_avgRainfall <- garch_jagung$avg_rainfall
```
...
Variabel Produksi jagung dan Curah Hujan Rata-rat tidak terdistribusi normal berdasarkan uji Shapiro-Wilk
Model GARCH sendiri tidak mensyaratkan distribusi normal untuk data atau residual

```{r}
# membagi data berdasarkan variabel avg_rainfall
set.seed(123)
data_index <- createDataPartition(garch_jagung$avg_rainfall, p=0.7, list = FALSE)
train_data_jagung <- garch_jagung[data_index,] #pelatih
train_data_jagung
test_data_jagung <- garch_jagung[-data_index,] #penguji
test_data_jagung
```

```{r}
#mendefinisikan karakteristik-karakteristik dari model GARCH yang akan digunakan untuk menganalisis deret waktu
...
#untuk melakukan estimasi atau penyesuaian model GARCH ke data deret waktu curah hujan (avg_rainfall) yang disimpan dalam objek train_data_jagung
garch_fit_jagung <- ugarchfit(data = train_data_jagung$avg_rainfall, spec = garch_model)
garch_fit_jagung
```
...
Sign Bias	          0.7096246	  0.47880236		
Negative Sign Bias	1.2362059	  0.21790012		
Positive Sign Bias	1.8142221	  0.07121269	*	
Joint Effect      	5.0768840	  0.16625312


```{r}
# Prediksi nilai volatilitas untuk data pengujian
volatility_forecast_jagung <- ugarchforecast(garch_fit_jagung, n.ahead = nrow(test_data_jagung))
volatility_forecast_jagung
```


```{r}
# Visualisasi nilai volatilitas dan produksi jagung
plot(test_data_jagung$Tahun, test_data_jagung$Produksi, type = "l", col = "blue", lty = 1, ylim = c(min(test_data_jagung$Produksi), max(test_data_jagung$Produksi)), xlab = "Tahun", ylab = "Produksi Jagung", main = "Produksi Jagung vs. Volatilitas Curah Hujan")
lines(test_data_jagung$Tahun, volatility_forecast_jagung@forecast$seriesFor, col = "red", lty = 2)
legend("topright", legend = c("Produksi Jagung", "Volatilitas Curah Hujan"), col = c("blue", "red"), lty = 1:2)
```

# Menghitung koefisien korelasi Pearson
```{r}
# Analisis korelasi antara curah hujan dan produksi jagung
correlation_produksi_jagung <- cor(test_data_jagung$avg_rainfall, test_data_jagung$Produksi)
cat ("Koefisien Korelasi Pearson Rata-rata Curah hujan dan Produksi:",correlation_produksi_jagung)

```
Koefisien Korelasi Pearson Rata-rata Curah hujan dan Produksi: 0.06639823


```{r}
# Statistik deskriptif untuk produksi jagung
summary(test_data_jagung$Produksi)

```
 Hasil coding : 
 Min.    1st Qu.  Median  Mean  3rd Qu.    Max. 
    283   31208   78557  391025  589273 2525896 
```{r}
# Statistik deskriptif untuk curah hujan
summary(test_data_jagung$avg_rainfall)
```
 Hasil coding: 
 Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
  5.211   7.082   7.774   7.763   8.468  10.049 


```{r}
# Scatter plot antara curah hujan dan produksi jagung
plot(test_data_jagung$avg_rainfall, test_data_jagung$Produksi, col = "blue", pch = 16, main = "Scatter Plot: Curah Hujan vs. Produksi Jagung", xlab = "Curah Hujan", ylab = "Produksi Jagung")

```

```{r}
# Analisis regresi (contoh menggunakan model linear sederhana)
regression_model <- lm(Produksi ~ avg_rainfall, data = test_data_jagung)
summary(regression_model)
```
 Estimate Std. Error t value Pr(>|t|)
(Intercept)   -230688     364348  -0.633    0.528
avg_rainfall    65463      46798   1.399    0.166


#uji perbandingan coding
```{r}
# Pilih variabel yang akan diprediksi
garch_jagung <- Sumatera_jagung[, c("Tahun" ,"Produksi", "avg_rainfall")]
garch_jagung
```
Menyertakan variabel eksternal dalam model adalah suatu cara untuk memeriksa apakah ada hubungan antara variabel tersebut dan volatilitas yang diobservasi.
```{r}
# Spesifikasi model GARCH dengan variabel exogenous (curah hujan)
garch_spec_exo <- ugarchspec(variance.model = list(model = "sGARCH", garchOrder = c(1, 1)),
                             external.regressors = matrix(garch_jagung$avg_rainfall, ncol = 1))
...
Dengan memodelkan volatilitas curah hujan, dapat mengeksplorasi apakah fluktuasi dalam curah hujan berkorelasi dengan fluktuasi dalam produksi jagung.
```{r}
garch_fit_exo <- ugarchfit(data = train_data_jagung$variabel_volatilitas, spec = garch_spec_exo)
```
...
## a. GARCH terhadap produktivitas jagung Sumatera

```{r}
# Pilih variabel yang akan diprediksi

garch_jagung_pt <- Sumatera_jagung[, c("Tahun" ,"Produktivitas", "avg_rainfall")]
garch_jagung_pt
```
...
# membagi data berdasarkan variabel avg_rainfall
set.seed(123)
data_index_jagung <- createDataPartition(garch_jagung_pt$avg_rainfall, p=0.7, list = FALSE)
train_data_jagung2 <- garch_jagung_pt[data_index,] #pelatih
train_data_jagung2
test_data_jagung2 <- garch_jagung_pt[-data_index,] #penguji
test_data_jagung2
```
...
# Estimasi model GARCH
#untuk melakukan estimasi atau penyesuaian model GARCH ke data deret waktu curah hujan (avg_rainfall) yang disimpan dalam objek train_data_jagung
garch_fit_jagung2 <- ugarchfit(data = train_data_jagung2$avg_rainfall, spec = garch_model)
garch_fit_jagung2
```
output:
                     t-value    prob     sig
...
```{r}
# Prediksi nilai volatilitas untuk data pengujian
volatility_forecast_jagung2 <- ugarchforecast(garch_fit_jagung2, n.ahead = nrow(test_data_jagung2))
volatility_forecast_jagung2
```


```{r}
# Visualisasi nilai volatilitas dan produksi jagung
plot(test_data_jagung2$Tahun, test_data_jagung2$Produktivitas, type = "l", col = "blue", lty = 1, ylim = c(min(test_data_jagung2$Produktivitas), max(test_data_jagung2$Produktivitas)), xlab = "Tahun", ylab = "Produksi Jagung", main = "Produksi Jagung vs. Volatilitas Curah Hujan")
lines(test_data_jagung2$Tahun, volatility_forecast_jagung2@forecast$seriesFor, col = "red", lty = 2)
legend("topright", legend = c("Produktivitas Jagung", "Volatilitas Curah Hujan"), col = c("blue", "red"), lty = 1:2)
```

# Menghitung koefisien korelasi Pearson
```{r}
# Analisis korelasi antara curah hujan dan produktivitas jagung
correlation_produktivitas_jagung2 <- cor(test_data_jagung2$avg_rainfall, test_data_jagung2$Produktivitas)
cat ("Koefisien Korelasi Pearson Rata-rata Curah hujan dan Produktivitas:",correlation_produktivitas_jagung2)

```
Koefisien Korelasi Pearson Rata-rata Curah hujan dan Produktivitas: 0.1224184


```{r}
# Statistik deskriptif untuk produktivitas jagung
summary(test_data_jagung2$Produktivitas)

```
 Hasil coding   
 Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
  14.49   22.91   32.85   37.55   54.79   69.80 
 
 
```{r}
# Statistik deskriptif untuk curah hujan
summary(test_data_jagung2$avg_rainfall)
```
 Hasil coding: 
 Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
  4.662   7.102   7.795   7.700   8.503  10.049 
  

```{r}
# Scatter plot antara curah hujan dan produktivitas jagung
plot(test_data_jagung2$avg_rainfall, test_data_jagung2$Produktivitas, col = "blue", pch = 16, main = "Scatter Plot: Curah Hujan vs. Produktivitas Jagung", xlab = "Curah Hujan", ylab = "Produktivitas Jagung")

```

```{r}
# Analisis regresi (contoh menggunakan model linear sederhana)
regression_jagung2 <- lm(Produktivitas ~ avg_rainfall, data = train_data_jagung2)
summary(regression_jagung2)
```
