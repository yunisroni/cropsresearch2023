...
splitIndex <- createDataPartition(data_var_jagung$Produksi, p = 0.7, list = FALSE)
data_train <- data_var_jagung[splitIndex, ]  # Data pelatihan (70%)
data_test <- data_var_jagung[-splitIndex, ]  # Data pengujian (30%)
dim(data_train)
dim(data_test)
```
...
predictions_rf <- predict(var_model_jagung_Sumatera, data_test)
predictions_rf
```
...
```{r}
irf_plot_jagung1 <- irf(var_model_jagung_Sumatera, impulse = "avg_rainfall", response = c("Produktivitas"), n.ahead = 15)
plot(irf_plot_jagung1)
```

```{r}
irf_plot_jagung2 <- irf(var_model_jagung_Sumatera, impulse = "avg_rainfall", response = c("Produksi"), n.ahead = 15)
plot(irf_plot_jagung2)
```
...
```{r}
set.seed(123) # Set seed agar hasil dapat direproduksi

split_ratio <- 0.7 # Rasio pembagian (70% untuk set pelatihan, 30% untuk set pengujian)

# Menggunakan sample.split untuk membagi data
split <- sample.split(ts_data_jagung_Sumatera[, "Produksi"], SplitRatio = split_ratio)

# Membuat set pelatihan dan pengujian berdasarkan pembagian di atas
train_data <- subset(ts_data_jagung_Sumatera, split == TRUE)
test_data <- subset(ts_data_jagung_Sumatera, split == FALSE)

```

```{r}
# Melakukan Prediksi terhadap Data Pengujian
predictions_var <- predict(var_model_jagung_Sumatera, test_data)
predictions_var
```

```{r}
# frekuensi data adalah tahunan
frekuensi_tahunan <- 1

# Menghitung respons impuls untuk 5 langkah ke depan
irf_jagung <- irf(var_model_jagung_Sumatera, impulse = "avg_rainfall", response = "Produktivitas", n.ahead = 5)
irf_jagung

```
*Kesimpulan*
Pada langkah pertama (1), respons impuls positif menunjukkan bahwa peningkatan satu unit dalam curah hujan ("avg_rainfall") diikuti oleh peningkatan sekitar 0.13 unit dalam "Produktivitas" jagung.

Pada langkah-langkah berikutnya (2 hingga 6), respons impuls negatif menunjukkan bahwa efek positif awal kemungkinan akan berkurang atau berbalik.

```{r}
# Plot respons impuls
plot(irf_jagung, main = "Impulse Response Function: avg_rainfall -> Produktivitas", xlab = "Langkah waktu", ylab = "Respons Impuls")
```


```{r}
forecast_jagung <- predict(var_model_jagung_Sumatera, n.ahead = 5)
forecast_jagung
```

```{r}
forecast_values <- predict(var_model_jagung_Sumatera, n.ahead = 10)
forecast_values
```


```{r}
# Ambil nilai aktual dari data time series
actual_values <- window(ts_data_jagung_Sumatera, start = end(ts_data_jagung_Sumatera) - 10 + 1)
actual_values

```

```{r}
# Ambil nilai prediksi untuk variabel Produksi
forecast_produksi <- forecast_values$fcst$Produksi
forecast_produksi
```
...
```{r}
# Menghitung koefisien korelasi Pearson
kolerasi_kedelai_Sumatera3 <- cor(data_var_kedelai_Sumatera$Produksi, data_var_kedelai_Sumatera$Produktivitas)
cat ("Koefisien Korelasi Pearson Produksi dan Produksi:", kolerasi_kedelai_Sumatera3)
```
Koefisien Korelasi Pearson Luas panen dan Produksi: -0.034
...
garch_jagung <- Sumatera_jagung[, c("Produksi", "avg_rainfall")]
garch_jagung
```
...
```{r}
ts_jagung_garch <- ts(garch_jagung, start = c(1993,1), frequency = 1)
ts_jagung_garch
```

```{r}
plot(ts_jagung_garch)
```

```{r}
class(ts_jagung_garch)
...
str(ts_jagung_garch)
```
Model GARCH dirancang khusus untuk menangani volatilitas heteroskedastis dalam deret waktu stasioner,lebih baik jika variabel yang di modelkan bersifat stasioner. Deret waktu non-stasioner seringkali sulit untuk dimodelkan secara akurat karena karakteristiknya yang berubah-ubah.
```{r}
# Uji Augmented Dickey-Fuller (ADF) pada variabel Produksi
adf_test_result1 <- adf.test(ts_jagung_garch[, "Produksi"])
print(adf_test_result1)
```
Hasil uji Augmented Dickey-Fuller (ADF) menunjukkan bahwa p-value adalah 0.01, yang lebih kecil dari tingkat signifikansi 0.05.dapat menyimpulkan bahwa data variabel Produksi cenderung stasioner.


```{r}
# Uji Augmented Dickey-Fuller (ADF) pada variabel Produksi
adf_test_result2 <- adf.test(ts_jagung_garch[, "avg_rainfall"])
print(adf_test_result2)
```


Hasil uji Augmented Dickey-Fuller (ADF) menunjukkan bahwa p-value adalah 0.01595 dapat menyimpulkan bahwa data variabel avg_rainfall adalah stasioner.
...
acf(ts_jagung_garch[, "avg_rainfall"], main="Autocorrelation Function Rainfall")
pacf(ts_jagung_garch[, "avg_rainfall"], main="Partial Autocorrelation Function Rainfall")

acf(ts_jagung_garch[, "Produksi"], main="Autocorrelation Function Produksi")
pacf(ts_jagung_garch[, "Produksi"], main="Partial Autocorrelation Function Produksi")
```
ACF dan PACF untuk "avg_rainfall" (Curah Hujan Rata-rata):

ACF (Autocorrelation Function): Memberikan gambaran tentang seberapa kuat hubungan antara curah hujan pada waktu tertentu dengan curah hujan pada waktu sebelumnya.
PACF (Partial Autocorrelation Function): Menunjukkan hubungan langsung antara curah hujan pada waktu tertentu dengan curah hujan pada waktu sebelumnya, setelah menghilangkan pengaruh waktu-waktu di antaranya.

ACF dan PACF untuk "Produksi" (Produksi Jagung):

ACF (Autocorrelation Function): Memberikan gambaran tentang seberapa kuat hubungan antara produksi jagung pada waktu tertentu dengan produksi jagung pada waktu sebelumnya.
PACF (Partial Autocorrelation Function): Menunjukkan hubungan langsung antara produksi jagung pada waktu tertentu dengan produksi jagung pada waktu sebelumnya, setelah menghilangkan pengaruh waktu-waktu di antaranya.
...
# Menggunakan sample.split untuk membagi data
split <- sample.split(ts_jagung_garch[, "Produksi"], SplitRatio = split_ratio)

# Membuat set pelatihan dan pengujian berdasarkan pembagian di atas
train_data <- subset(ts_jagung_garch, split == TRUE)
test_data <- subset(ts_jagung_garch, split == FALSE)
...
```{r}
# Spesifikasi model GARCH(2.2)
spec2 <- ugarchspec(variance.model = list(model = "sGARCH", garchOrder = c(2, 2)),
                   mean.model = list(armaOrder = c(0, 0)))
```

```{r}
# Membuat seri waktu dari set pelatihan
# tujuan untuk Menyiapkan data pelatihan dalam format seri waktu yang diperlukan oleh fungsi ugarchfit.
ts_train_data <- ts(train_data[, "Produksi"], start = c(1993, 1), frequency = 1)
...
# Menyesuaikan model GARCH
# Tujuan untuk Melatih model GARCH pada data produksi jagung dari set pelatihan untuk mengevaluasi dan memodelkan volatilitas.
garch_model <- ugarchfit(data = ts_train_data, spec = spec) 
...
alpha1: 4.5300e-03
beta1: 9.9447e-01
Parameter-parameter ini menggambarkan komponen volatilitas model GARCH. Secara khusus, alpha1 dan beta1 mencerminkan bagaimana volatilitas dipengaruhi oleh informasi volatilitas di periode sebelumnya. tidak terdapat p-valuenya karena hasilnya NA

Weighted ARCH LM Tests:

Uji autoregresif terhadap kuadrat residual.
Hasil uji ini memberikan informasi tentang adanya struktur autoregresif pada kuadrat residual, yang dapat diartikan sebagai indikator volatilitas kondisional.

```{r}
# Melakukan perkiraan volatilitas di masa depan. Ini dapat membantu memahami pola volatilitas yang mungkin terjadi.
forecast_volatility <- ugarchforecast(garch_model, n.ahead = 10)
print(forecast_volatility)
```
### Melakukan uji diagnostik lanjutan pada model untuk memastikan kecocokan model
```{r}
# Uji autokorelasi pada residual

acf_test <- acf(residuals(garch_model))
print(acf_test)
...
```{r}
# Menghitung koefisien korelasi Pearson
kolerasi_jagung_Garch <- cor(garch_jagung$Produksi, garch_jagung$avg_rainfall)
cat ("Koefisien Korelasi Pearson Luas panen dan Produksi:", kolerasi_jagung_Garch)
```
Koefisien Korelasi Pearson Luas panen dan Produksi: 0.084


### Uji regresi linier jagung
```{r}
# Membuat model regresi linier
regresi_jagung_garch <- lm(Produksi ~ avg_rainfall , data = garch_jagung)

# Ringkasan hasil model
summary(regresi_jagung_garch)
```
avg_rainfall nilai estimasinya adalah sedangkan p-valuenya   0.0108 *
dengan f-statistik p-valuenya adalah 0.01078
