---
title: "Prediksi Variabel Yang Mempengaruhi Produksi Kacang Kedelai Masa Depan - Aceh"
...
## a.Import Data
```{r}
pangan <- read_excel("Dataset Kacang Kedelai Indonesia/pangan_Indonesia.xlsx")
```
...
## c. Jumlah Produksi Kacang Kedelai Per Provinsi
```{r}
...
## d. Tren Produksi Kacang Kedelai Per Provinsi
```{r}
...
## e. Perbandingan Produksi Kacang Kedelai Per Provinsi
```{r}
...
## f. Perbandingan Produktivitas Kacang Kedelai Per Provinsi
```{r}
# Turn off dplyr summarise() warning
options(dplyr.summarise.inform = FALSE)
...
## g. Korelasi Luas Panen, Produktivitas dengan Produksi Kacang Kedelai
```{r}
...
```{r}
# Visualisasi korelasi produktifitas dan produksi Kacang Kedelai
plot_kacangkedelai_5 <- ggplot(KacangKedelai, aes(x = Produktivitas, y = Produksi, color = Provinsi)) +
  geom_point() +
...
```{r}
# Hitung Pearson correlation coefficient
correlation_kacangkedelai2 <- cor(KacangKedelai$Produktivitas, KacangKedelai$Produksi)

# Nilai correlation coefficient
cat("Koefisien Korelasi Pearson Luas Panen dan Produksi:", correlation_kacangkedelai2)
```
Berdasarkan grafik diatas bisa lihat bahwa ada korelasi antara luas panen dan produksi. Hubungan korelasi adalah positif dengan nilai korelasi sebesar 0.22; Nilai korelasi sebesar 0.22 menunjukkan hubungan positif yang relatif lemah. Ini berarti bahwa meskipun ada hubungan antara produktivitas dan produksi, hubungannya tidak sangat kuat. Namun, itu masih dapat dianggap sebagai hubungan yang signifikan.


## h. Filter data jagung

```{r}
KacangKedelaiAceh <- filter(pangan_clean, Jenis_Komoditi == "Kedelai", Provinsi == "ACEH")
KacangKedelaiAceh
...
train_data_index_lp <- createDataPartition(KacangKedelaiAceh$Luas_Panen, p=0.7, list = FALSE)
train_data_lp <- KacangKedelaiAceh[train_data_index_lp,]
train_data_lp
test_data_lp <- KacangKedelaiAceh[-train_data_index_lp,]
test_data_lp
```
...
```{r}
# membagi data berdasarkan variabel Produksi
train_data_index_pr <- createDataPartition(KacangKedelaiAceh$Produktivitas, p=0.7, list = FALSE)
train_data_pr <- KacangKedelaiAceh[train_data_index_pr,]
train_data_pr
test_data_pr <- KacangKedelaiAceh[-train_data_index_pr,]
test_data_pr
```
