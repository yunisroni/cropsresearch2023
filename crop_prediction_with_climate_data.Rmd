...
```{r}
# Import dataset
cropData <- Dataset_Sumatera_2006_2015
glimpse(cropData)
```
...
cropData$Produktivitas <- as.numeric(cropData$Curah_hujan)
glimpse(cropData)
```
...
Ada variabel Produk 28 NA's dan Luas_Panen 28 NA's. Data NA'S akan dihapus agar proses selanjutnya dapat dilakukan dengan baik
...
```
Berdasarkan matrik korelasi diatas bisa disimpulkan bahwa korelasi yang sangat kuat yaitu korelasi *Rainfall* dengan *Productivity*. Kemudian korelasi yang kuat antara *Area* dengan *Production*. sedangkan untuk korelasi variabel lain termasuk korelasi sedang dan lemah.
...
Ada korelasi positif yang kuat (0.51) antara variabel "Production" dan "Area". Ini menunjukkan bahwa luas tanam (Area) berkorelasi positif dengan produksi tanaman.

Korelasi Negatif yang Signifikan:
Terdapat korelasi negatif yang signifikan (-0.47) antara variabel "Area" dan "Wind_Velocity". Hal ini menunjukkan bahwa kecepatan angin (Wind_Velocity) memiliki hubungan negatif dengan luas tanam (Area).

Variabel yang Tidak Berkorelasi Kuat:
Variabel "Production" tidak memiliki korelasi yang kuat dengan variabel "Productivity" (0.05), "Humidity" (0.002), dan "Temperature" (0.027). Ini menunjukkan bahwa produksi tanaman tidak secara langsung berkorelasi kuat dengan produktivitas, kelembaban, atau suhu.

Korelasi Negatif Moderat:
Terdapat korelasi negatif moderat antara variabel "Productivity" dan "Sunshine_Duration" (-0.36), serta antara "Sunshine_Duration" dan "Rainfall" (-0.36). Ini menunjukkan bahwa semakin banyak sinar matahari (Sunshine_Duration), semakin rendah produktivitas dan curah hujan (Rainfall).

Variabel yang Tidak Berkorelasi:
Variabel "Humidity" tidak memiliki korelasi yang kuat dengan variabel lainnya dalam matriks ini. Korelasi dengan variabel lainnya adalah 0.002 hingga 0.19.
...
padi_select <- padi_data %>% 
  dplyr::select(Production_log, Area_log, Productivity, Wind_Velocity, Humidity, Temperature, Sunshine_Duration, Rainfall)
...
# Buat correlogram
ggpairs(padi_select, 
        columns = 1:8,  # Pilih kolom yang akan diplot
...
colnames(padi_data)[colSums(padi_data == 0) > 0]
```

```{r}
colSums(is.na(padi_data))
```

```{r}
# Membuat data frame panjang untuk variabel yang berbeda
df_long <- pivot_longer(padi_data, cols = c(Production, Wind_Velocity, Humidity, Temperature, Sunshine_Duration, Rainfall, Area), names_to = "Variable", values_to = "Value")
...
lm_model_1 <- lm(Production_log ~ Rainfall + Humidity + Wind_Velocity + Temperature + Sunshine_Duration + Area, data = padi_data)

summary(lm_model_1)
```

Dalam model 1, variabel yang paling signifikan dalam menjelaskan variasi dalam produksi adalah *Area_log*

```{r}
# Model Prediksi Productivity
lm_model_2 <- lm(Productivity ~ Production_log + Rainfall + Humidity + Wind_Velocity + Temperature + Sunshine_Duration + Area, data = padi_data)

summary(lm_model_2)
```

Dalam model 2 terlihat bahwa ada pengaruh sangat siginfikan antara *Rainfall* dengan *Productivity*

```{r}
# Model Prediksi Area
lm_model_3 <- lm(Area ~ Production_log + Productivity + Rainfall + Humidity + Wind_Velocity + Temperature + Sunshine_Duration, data = padi_data)

summary(lm_model_3)
```

Dalam model 3 ini, variabel Production dan Wind_Velocity berpengaruh signifikan terhadap Area

Berdasarkan model 2 bisa disimpulkan bahwa: Rainfall berpengaruh signifikan terhadap Productivity, tapi sepertinya terlalu sempurna dalam memfitting data, karena nilai regresi = 1. Maka perlu dilakukan analisis lebih lanjut yaitu analisis multikolineritas atau overfitting.
...
ridge_model <- glmnet(
  x = as.matrix(padi_data[, c("Area", "Productivity", "Rainfall", "Humidity", "Wind_Velocity", "Temperature", "Sunshine_Duration")]),
  y = as.vector(padi_data$Production_log),
...
Berdasarkan hasil analisis model regresi, dapat diambil beberapa kesimpulan utama:

1.  **Variabel yang Signifikan:**
    -   Variabel Area_log, Productivity, Rainfall, Temperature, dan Sunshine_Duration memiliki koefisien yang signifikan secara statistik dan memberikan kontribusi dalam menjelaskan variasi Produksi padi.
2.  **Pengaruh Variabel:**
    -   Variabel Area_log, Productivity, Rainfall, dan Sunshine_Duration memiliki pengaruh positif terhadap Produksi padi. Artinya, semakin besar area, produktivitas, curah hujan, suhu, dan durasi sinar matahari, semakin besar produksi yang diharapkan.

    -   Variabel Humidity, Temperature dan Wind_Velocity memiliki pengaruh negatif terhadap Produksi padi. Artinya, semakin tinggi kelembapan, temperatur dan kecepatan angin, semakin kecil produksi yang diharapkan.
3.  **Kesesuaian Model:**
    -   Dengan R-squared mencapai 1, model regresi ini mampu menjelaskan seluruh variasi dalam Produksi padi. Namun, perlu diingat bahwa "perfect fit" dapat menimbulkan kekhawatiran akan overfitting, di mana model mungkin terlalu sesuai dengan data pelatihan dan kurang dapat diterapkan pada data baru.
...
# Data
X <- model.matrix(Production_log ~ Area + Productivity + Rainfall + Humidity + Wind_Velocity + Temperature + Sunshine_Duration, data = padi_data)
y <- padi_data$Production_log
...
Berdasarkan validasi yang sudah dilakukan, dapat disimpulkan bahwa menggunakan 7 variabel yang sudah analisis sebelumnya masih relevan dalam memprediksi *Production*. 7 variabel tersebut meliputi: *Area*, *Productivity*, *Rainfall*, *Humidity*, *Wind_Velocity*, *Temperature*, dan *Sunshine_Duration*

## PCA Analysis

```{r}
#Pre-Processing Data
dataPre <- preProcess(padi_data[,c(5, 7:13)], method = c("center", "scale"))

#Normalisasi 
VARpadi <- predict(dataPre, padi_data[, c(5, 7:13)])
summary(VARpadi)
```
...
partition <- createDataPartition(padi_data$Production_log, p = 0.7, list = FALSE)
data_training <- padi_data[partition, ]
data_testing <- padi_data[-partition, ]
```
...
model_glm <- glm(Production_log ~ Rainfall + Humidity + Wind_Velocity + Temperature + Sunshine_Duration + Area_log, data = data_training, family = gaussian())
...
```{r}
# Membuat model RF
model_rf <- randomForest(Production_log ~ Rainfall + Humidity + Wind_Velocity + Temperature + Sunshine_Duration + Area_log, data = data_training, ntree = 1000)
...
response_variable <- "Production_log"
predictor_variables <- c("Area_log", "Productivity", "Rainfall", "Humidity", "Wind_Velocity", "Temperature", "Sunshine_Duration")
...
```{r}
# Membuat model SVR
model_svr <- svm(train_data_scaled[, predictor_variables], train_data[[response_variable]], kernel = "linear", cost = 1)
...
X_train <- data.matrix(data_training[, c("Rainfall", "Humidity", "Wind_Velocity", "Temperature", "Sunshine_Duration", "Area_log")])
y_train <- data_training$Production_log
X_test <- data.matrix(data_testing[, c("Rainfall", "Humidity", "Wind_Velocity", "Temperature", "Sunshine_Duration", "Area_log")])
...
gbm_model <- gbm(
  ...
  n.trees = 100,  # Jumlah pohon
  interaction.depth = 3,  # Kedalaman interaksi
...
```{r}
# Melakukan prediksi pada set pengujian
predictions_gbm <- predict(gbm_model, newdata = data_testing, n.trees = 100, type = "response")

```

```{r}
# Visualisasi hasil prediksi
result_data_gbm <- data.frame(Production = data_testing$Production_log, Predictions = predictions_gbm)
...
```{r}
# Buat grafik garis
ggplot(data = result_data_gbm, aes(x = 1:length(data_testing$Production_log))) +
  geom_line(aes(y = data_testing$Production_log, color = "Actual"), size = 1) +
...
# Hitung MAE
mae_value_gbm <- mae(data_testing$Production_log, predictions_gbm)

# Hitung MSE
mse_value_gbm <- mse(data_testing$Production_log, predictions_gbm)

# Hitung RMSE
rmse_value_gbm <- rmse(data_testing$Production_log, predictions_gbm)

# Hitung MAPE
mape_value_gbm <- mape(data_testing$Production_log, predictions_gbm)
...
```{r}
# Visualisasi hasil prediksi
result_data_knn <- data.frame(Production = data_testing$Production_log, Predictions = predictions_knn)
...
```{r}
# Buat grafik garis
ggplot(data = result_data_knn, aes(x = 1:length(data_testing$Production_log))) +
  geom_line(aes(y = data_testing$Production_log, color = "Actual"), size = 1) +
...
# Hitung MAE
mae_value_knn <- mae(data_testing$Production_log, predictions_knn)

# Hitung MSE
mse_value_knn <- mse(data_testing$Production_log, predictions_knn)

# Hitung RMSE
rmse_value_knn <- rmse(data_testing$Production_log, predictions_knn)

# Hitung MAPE
mape_value_knn <- mape(data_testing$Production_log, predictions_knn)
...
```{r}
# Visualisasi hasil prediksi
result_data_ann <- data.frame(Production = test_data$Production_log, Predictions = predictions_ann)
...
```{r}
# Buat grafik garis
ggplot(data = result_data_ann, aes(x = 1:length(test_data$Production_log))) +
  geom_line(aes(y = test_data$Production_log, color = "Actual"), size = 1) +
...
# Hitung MAE
mae_value_ann <- mae(test_data$Production_log, predictions_ann)

# Hitung MSE
mse_value_ann <- mse(test_data$Production_log, predictions_ann)

# Hitung RMSE
rmse_value_ann <- rmse(test_data$Production_log, predictions_ann)

# Hitung MAPE
mape_value_ann <- mape(test_data$Production_log, predictions_ann)
...
```{r}
# Visualisasi hasil prediksi
result_data_dt <- data.frame(Production = test_data$Production_log, Predictions = predictions_dt)
...
```{r}
# Buat grafik garis
ggplot(data = result_data_dt, aes(x = 1:length(test_data$Production_log))) +
  geom_line(aes(y = test_data$Production_log, color = "Actual"), size = 1) +
...
# Hitung MAE
mae_value_dt <- mae(test_data$Production_log, predictions_dt)

# Hitung MSE
mse_value_dt <- mse(test_data$Production_log, predictions_dt)

# Hitung RMSE
rmse_value_dt <- rmse(test_data$Production_log, predictions_dt)

# Hitung MAPE
mape_value_dt <- mape(test_data$Production_log, predictions_dt)
...
long_sort <- min(length(data_testing$Production_log),length(predicted_glm), length(predictions_rf), length(predictions_svr), length(predictions_xgb), length(predictions_gbm), length(predictions_knn), length(predictions_ann), length(predictions_dt), 28)
