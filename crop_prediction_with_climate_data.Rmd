...
library(tidyr)
padi_select_t <- padi_select %>% 
  dplyr::select(Area, Production, Productivity, Humidity, Wind_Velocity, Sunshine_Duration, Temperature, Rainfall)

padi_select_tidy <- gather(padi_select_t, key = "variable", value = "value")

# Create histogram for each variable in one page
ggplot(data = padi_select_tidy, aes(x = value)) +
  geom_histogram(binwidth = 5, fill = "blue", color = "black", alpha = 0.7) +
  labs(title = "Histogram of Data Distribution",
       x = "Values",
       y = "Frequency") +
  theme_minimal() +
...
```
```{r}
nonzero_coef <- predict(best_ridge_model, type = "nonzero")
nonzero_coef
```
...
```{r}
# Hyperparamater Random Search

# Install dan load library
# install.packages(c("glm2", "boot"))
library(glm2)
library(boot)
...
# Fungsi untuk membuat model GLM dengan glm2
create_glm2_model <- function(formula, data) {
  model <- glm2(formula, data = data)
  return(model)
}

# Fungsi untuk melakukan cross-validation pada model GLM dengan glm2
cv_glm2 <- function(data, indices, formula) {
  train_data <- data[indices, ]
  test_data <- data[-indices, ]
  
  model <- create_glm2_model(formula, train_data)
  
  # Prediksi pada data uji
  predictions <- predict(model, newdata = test_data, type = "response")
  
  # Menghitung RMSE (sebagai contoh, bisa diganti dengan metrik lain)
  rmse <- sqrt(mean((test_data$Y - predictions)^2))
  
  return(rmse)
}

# Contoh formula untuk model GLM
formula <- Y ~ X1 + X2

# Cross-validation dengan 5-fold
cv_result <- cvms(data, cv_glm2, formula = formula, m = 5)
...
model_glm <- glm(Production ~ Rainfall + Humidity + Wind_Velocity + Temperature + Sunshine_Duration + Area + Productivity, data = data_training, family = gaussian(link = "identity"), control = list(epsilon = 1e-8, maxit =100))
...
train_data <- padi_rs[train_indices, ]
test_data <- padi_rs[-train_indices, ]

# Menetapkan variabel respons dan prediktor
response_variable <- "Y"
predictor_variables <- setdiff(names(train_data), response_variable)

# Membuat grid hyperparameter untuk Random Search pada Random Forest
hyperparameter_grid <- data.frame(
  mtry = sqrt(length(predictor_variables))  # Parameter mtry untuk jumlah prediktor yang diambil acak berdasarkan sqrt
  #ntree = sample(50:200, 10),            # Contoh variasi nilai ntree
  #min_samples_split = c(2, 5, 10),       # Contoh variasi nilai min_samples_split
  # min_samples_leaf = c(1, 2, 4)
)

# Menetapkan kontrol untuk Random Search
control <- trainControl(
  method = "cv",  # Cross-validation sebagai metode evaluasi
  number = 5,     # Jumlah lipatan cross-validation
  search = "random",  # Metode pencarian Random Search
  verboseIter = TRUE
)

# Melakukan Random Search dengan model Random Forest Regression
model <- train(
...
# Evaluasi model (gantilah dengan metrik evaluasi yang sesuai untuk regresi)
rmse <- sqrt(mean((test_data$Y - predictions)^2))
cat("RMSE:", rmse, "\n")
...
```{r}
# Membuat model RF
model_rf <- randomForest(Production ~ Rainfall + Humidity + Wind_Velocity + Temperature + Sunshine_Duration + Area + Productivity, data = data_training, ntree = 200, mtry = floor(sqrt(ncol(data_training))), importance = TRUE)
...
train_data <- padi_svr[train_indices, ]
test_data <- padi_svr[-train_indices, ]
...
```{r}
# Membuat model SVR
model_svr <- svm(train_data_scaled[, predictor_variables], train_data[[response_variable]], kernel = "linear", cost = 1.33548, epsilon = 0.1, gamma = 0.01471221)
...
# Split dataset menjadi data pelatihan dan data uji
train_indices <- sample(seq_len(n), size = 0.8 * n)
train_data <- padi_xgb[train_indices, ]
test_data <- padi_xgb[-train_indices, ]

# Menetapkan variabel respons dan prediktor
response_variable <- "Y"
predictor_variables <- setdiff(names(train_data), response_variable)

# Membuat grid hyperparameter untuk Random Search pada XGBoost Regression
hyperparameter_grid <- expand.grid(
  eta = runif(10, 0.01, 0.3),        # Contoh variasi nilai learning rate
  max_depth = sample(3:10, 10),       # Contoh variasi nilai max_depth
  nrounds = sample(50:100, 10)        # Contoh variasi nilai nrounds
)

# Melakukan Random Search dengan model XGBoost Regression
best_rmse <- Inf
best_xgb_model <- NULL

for (i in 1:nrow(hyperparameter_grid)) {
  # Membuat model XGBoost dengan parameter tertentu
  xgb_model <- xgboost(
    data = as.matrix(train_data[, predictor_variables]),
    label = train_data[[response_variable]],
  ...
  # Prediksi pada data uji
  predictions <- predict(xgb_model, newdata = as.matrix(test_data[, predictor_variables]))
...
# Prediksi pada data uji menggunakan model terbaik
predictions <- predict(best_xgb_model, newdata = as.matrix(test_data[, predictor_variables]))
...
model_xgb <- xgboost(data = xgb_data_train, nrounds = 100, objective = "reg:squarederror", booster = "gbtree", eta = 0.3)
```
...
gbm_model <- gbm(
  ...
  shrinkage = 0.1,  # Shrinkage atau learning rate
...
decision_tree_model <- rpart(
  ...
  data = train_data,
  method = "anova",
  control = rpart.control(minsplit = 2, cp = 0.007, minleaf = 1)  # Hyperparameter control
)
```
...
```{r}
write.csv(model_performance, file = "matrik_1.csv", row.names = TRUE)
```
