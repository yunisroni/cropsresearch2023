...
future_prophet_ts_ch <- make_future_dataframe(prophet_ts_ch, periods = 365*10)
...
forecast_subset <- forecast_ts_ch[forecast_ts_ch$ds >= as.Date("2021-01-01") & forecast_ts_ch$ds <= as.Date("2029-12-31"), ]

# Hanya ambil kolom Tahun dan yhat
forecast_yhat_2021_2029 <- forecast_subset[, c("ds", "yhat")]

# Print hasilnya
print(forecast_yhat_2021_2029)
```
```{r}
tail(forecast_yhat_2021_2029,10)
```



```{r}
yearly_ch_predict <- forecast_yhat_2021_2029 %>% 
  group_by(ds) %>% 
  summarise(
    jum_ch = sum(yhat)
  )
