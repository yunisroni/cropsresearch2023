---
title: "Prediksi Variabel Yang Mempengaruhi Produksi Jagung Masa Depan - DKI Jakarta"
...
```{r}
JagungDKIJakarta <- filter(pangan_clean, Jenis_Komoditi == "Jagung", Provinsi == "DKI JAKARTA")
JagungDKIJakarta
...
train_data_index_lp <- createDataPartition(JagungDKIJakarta$Luas_Panen, p=0.7, list = FALSE)
train_data_lp <- JagungDKIJakarta[train_data_index_lp,]
train_data_lp
test_data_lp <- JagungDKIJakarta[-train_data_index_lp,]
test_data_lp
```
...
future_prophet_ts_lp <- make_future_dataframe(prophet_ts_lp, periods = 365*14)
...
```{r}
# membagi data berdasarkan variabel Produksi
train_data_index_pr <- createDataPartition(JagungDKIJakarta$Produktivitas, p=0.7, list = FALSE)
train_data_pr <- JagungDKIJakarta[train_data_index_pr,]
train_data_pr
test_data_pr <- JagungDKIJakarta[-train_data_index_pr,]
test_data_pr
```
...
```{r}
# Melakukan Prediksi untuk produksi 10 tahun ke depan
future_prophet_ts_pr <- make_future_dataframe(prophet_ts_pr, periods = 365*15)
