...
ts_data1 <- ts(indonesia_jagung$Produksi, start = start(indonesia_jagung$Tahun), frequency = 1)
ts_data1
```
...
```{r}
# Fit the GARCH model
garch_fit <- ugarchfit(data = ts_data, spec = garch_model)
```
...
```
```{r}
# Daftarkan Avg_Rainfall dan Produksi menjadi dataframe
indonesia_garch <- data.frame(Produksi = indonesia_jagung$Produksi, Avg_Rainfall = indonesia_jagung$avg_Rainfall)
indonesia_garch

```
```{r}
#Format time series
ts_jagung_indonesia2 <- ts(indonesia_garch, start = c(1993,1), frequency = 1)
ts_jagung_indonesia2
```
...
```
```{r}
# Transformasikan Avg_Rainfall menjadi model GARCH
...
garch_fit <- ugarchfit(spec = garch_spec, data = ts_jagung_indonesia$Produksi, out.sample = 0, fit.control = list(eval.se = TRUE))
summary(garch_fit)
```

```{r}
# Estimasi parameter model GARCH
garch_fit2 <- ugarchfit(spec = garch_spec, data = ts_jagung_indonesia$Produksi, out.sample = 0, fit.control = list())
summary(garch_fit2)
```
