...
data_garch_jagung <- indonesia_jagung[, c("Produksi", "avg_Rainfall")]
data_garch_jagung
```
...
```{r}
#Format time series
ts_jagung_indonesia <- ts(data_garch_jagung, start = c(1993,1), frequency = 1)
ts_jagung_indonesia
```
...
adf_test_result2 <- adf.test(ts_jagung_indonesia[, "avg_Rainfall"])
...
```{r}
# Uji Augmented Dickey-Fuller (ADF) pada variabel avg_Rainfall
adf_test_result1 <- adf.test(data_garch_jagung$"Produksi")
...
acf(data_garch_jagung$avg_Rainfall, main="Autocorrelation Function 1")
pacf(data_garch_jagung$avg_Rainfall, main="Partial Autocorrelation Function 1")

acf(data_garch_jagung$Produksi, main="Autocorrelation Function 2")
pacf(data_garch_jagung$Produksi, main="Partial Autocorrelation Function 2")
```

```{r}
# Uji normalitas dengan Shapiro-Wilk
shapiro.test(data_garch_jagung$Produksi)

shapiro.test(data_garch_jagung$avg_Rainfall)
```
```{r}
transformed_dependen <- log(dependen_variable)
transformed_independen <- sqrt(independen_variable)
```

```{r}
## Penentuan Variabel Dependensi dan Independensi
dependen_variable <- data_garch_jagung$Produksi
independen_variable <- data_garch_jagung$avg_Rainfall
```


```{r}
set.seed(123) 
# Menentukan proporsi data yang akan menjadi set pelatihan
split_ratio <- 0.7  #  70% data pelatihan dan 30% data pengujian

# Membagi data menjadi set pelatihan dan pengujian
split <- sample.split(data_garch_jagung$avg_Rainfall, SplitRatio = split_ratio)

# Membuat set pelatihan dan pengujian berdasarkan pembagian di atas
train_data <- subset(data_garch_jagung, split == TRUE)
test_data <- subset(data_garch_jagung, split == FALSE)
```
...
fit1 <- ugarchfit(spec, data = train_data[, "avg_Rainfall"])
summary(fit)
```
```{r}
fit1
...
```{r}
fit2 <- ugarchfit(spec, data = test_data[, "avg_Rainfall"])
fit2
```
```{r}
fit3 <- ugarchfit(spec, data = train_data[, "Produksi"])
fit3
```

```{r}
fit4 <- ugarchfit(spec, data = test_data[, "Produksi"])
fit4
```
